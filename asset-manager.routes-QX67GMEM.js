...
var AssetBulkFormComponent = class _AssetBulkFormComponent extends AsyncHandler {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        ...
        const asset = await Gh(params.get("id")).catch(() => null);
        if (!asset) {
      ...
      if (params.get("group_id")) {
        ...
        const product = await Vh(params.get("group_id")).catch(() => null);
        if (!product) {
  ...
  async save() {
    ...
    const data = this.model();
    const list = await Kh(new Array(this.count()).fill(__spreadProps(__spreadValues({}, data), {
      zone_id: this._org.building.id
    }))).catch((e) => {
      this.loading.set("");
...
var AssetGroupFormComponent = class _AssetGroupFormComponent extends AsyncHandler {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        this.loading.set("Loading Product Details...");
        const product = await Vh(params.get("id")).catch(() => null);
        if (!product) {
...
//# sourceMappingURL=asset-manager.routes-QX67GMEM.js.map
