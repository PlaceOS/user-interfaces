...
function maybeRemoveStaleArrayFields(prevData, value, identitySymbol) {
  ...
  if (oldTracking && oldTracking.size > 0) {
    data ??= __spreadValues({}, prevData);
    for (const id2 of oldTracking) {
      data.byTrackingKey.delete(id2);
    }
  }
  return data;
}
  ...
  _syncDescribedByIds() {
    if (this._control) {
      ...
      if (existingDescribedBy) {
        ...
        toAssign = ids.concat(existingDescribedBy.filter((id2) => id2 && !exclude.includes(id2)));
      } else {
        toAssign = ids;
      }
...
var MatRadioButton = class _MatRadioButton {
  ...
  ngOnInit() {
    ...
    this._removeUniqueSelectionListener = this._radioDispatcher.listen((id2, name) => {
      if (id2 !== this.id && name === this.name) {
        this.checked = false;
      }
    });
  }
...
//# debugId=b62b8966-032f-575c-853e-8d0679489b43
//# sourceMappingURL=chunk-MIAW2KGF.js.map
