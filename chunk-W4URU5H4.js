...
var _BookingRulesFormComponent = class _BookingRulesFormComponent {
  constructor() {
    ...
    this.building_zones = this._org.active_building.pipe(filter((_) => !!_), switchMap((bld) => Da({ parent_id: bld.id }).pipe(catchError(() => of({ data: [] })))), map((res) => res.data), shareReplay(1));
...
var _BookingRulesModalComponent = class _BookingRulesModalComponent {
  constructor() {
    ...
    this.booking_rules = combineLatest([this._org.active_building, this.change]).pipe(filter(([_]) => !!_), switchMap(([bld]) => {
      return Xu(bld.id, `${this._data.type}_booking_rules`).pipe(catchError(() => of({ details: [] })));
...
//# sourceMappingURL=chunk-W4URU5H4.js.map
