...
async function queryBookings(q) {
  ...
  try {
    const list = await d(`${BOOKINGS_ENDPOINT}${query ? "?" + query : ""}`);
...
async function checkinBooking(id, state) {
  ...
  try {
    return new Booking(await S(`${BOOKINGS_ENDPOINT}/${encodeURIComponent(id)}/check_in?${query}`, ""));
  } catch (e) {
...
async function checkinBookingInstance(id, start_time, state) {
  const query = toQueryString({ state });
  try {
    return new Booking(await S(`${BOOKINGS_ENDPOINT}/${encodeURIComponent(id)}/check_in/${start_time}?${query}`, ""));
  } catch (e) {
    const body = await e.json();
    throw body.error || body.message || body;
  }
...
//# sourceMappingURL=chunk-VRRVJJKW.js.map
