...
var _EmailTemplatesStateService = class _EmailTemplatesStateService extends AsyncHandler {
  constructor() {
    ...
    this.template_definitions = combineLatest([
      ...
      this._change
    ]).pipe(filter(([bld]) => !!bld), switchMap(() => Iu(this._org.organisation.id, "email_template_fields").pipe(map((_) => {
    ...
    this.templates = combineLatest([
      ...
      Iu(this._org.organisation.id, "email_templates").pipe(map((_) => this._processTemplates(_, this._org.organisation.id)), catchError(() => of([]))),
      Iu(bld.id, "email_templates").pipe(map((_) => this._processTemplates(_, bld.id)), catchError(() => of([]))),
      region ? Iu(region.id, "email_templates").pipe(map((_) => this._processTemplates(_, region.id)), catchError(() => of([]))) : of([])
  ...
  async saveTemplate(template, old_zone = "") {
    ...
    if (template.id && old_zone) {
      const old_metadata = await lastValueFrom(Iu(old_zone, "email_templates"));
      if (old_metadata.details instanceof Array) {
        await lastValueFrom(Pu(old_zone, {
    ...
    const metadata = await lastValueFrom(Iu(template.zone_id, "email_templates"));
    ...
    await lastValueFrom(Pu(template.zone_id, {
      name: `email_templates`,
...
//# sourceMappingURL=chunk-MM4V7IQY.js.map
