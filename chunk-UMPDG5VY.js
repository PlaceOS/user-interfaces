...
var DesksStateService = class _DesksStateService extends AsyncHandler {
        ...
        try {
          ...
          if (zones.length && !params.zones.includes("All")) {
            const metadata = await Promise.all(zones.map(async (zone) => ({
              zone,
              details: await rc(zone, "desks").then((item) => item.details instanceof Array ? item.details : []).catch(() => [])
            })));
            list = metadata.flatMap(({ zone, details }) => details.map((item) => __spreadProps(__spreadValues({}, item), {
              zone: { id: zone }
            })));
          } else {
            const metadata = await hc(this._org.building?.id, { name: "desks" }).catch(() => []);
  ...
  async _checkAssignedDeskLimit(user_email, current_desk_id) {
    ...
    const levels = this._currentLevelList();
    const desks = this._settings.get("app.desks.use_assets") ? await queryDeskAssetsForZones(levels.map((level) => level.id)) : (await Promise.all(levels.map((level) => rc(level.id, "desks").then((metadata) => metadata.details instanceof Array ? metadata.details : []).catch(() => [])))).flat();
...
//# sourceMappingURL=chunk-UMPDG5VY.js.map
