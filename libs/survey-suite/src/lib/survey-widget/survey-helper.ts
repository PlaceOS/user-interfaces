import { Data, PieChartData } from "chartist";
import { QuestionType, UISurveyAnswer } from "../types";

export function parseSelectionAnswers(data:UISurveyAnswer[], choices?: string[]): Data<any>{
    if(!data?.length) return {labels:[], series:[]}
    const type = data[0].type;
    let map = {};
    let list = (type === QuestionType.Check_Box)
...
export function parseRatingAverage(data: UISurveyAnswer[],rateMax: number): PieChartData{
    const list = data.reduce((acc,curr) => (acc.push(curr.answer_json),acc),[]);
    const sum = list.reduce((acc, curr) => (acc + curr));
    const average = Math.round((sum / list.length) * 10) / 10;
    return {series:[
        {value: average, className:'stroke-green-600'}, 
        {value: rateMax - average, className: 'stroke-gray-300'}
    ]};
}