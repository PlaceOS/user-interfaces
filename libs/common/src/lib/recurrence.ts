...
function monthlyWeekdayStart(
    ...
    if (week < 0) {
        ...
        day_of_month =
            1 +
            ((day_of_week - first_day.getDay() + 7) % 7) +
            (week - 1) * 7;
    }
...
export function fromEventRecurrence(r: RecurrenceDetails): Recurrence {
    ...
    if (r.pattern === 'monthly' || r.pattern === 'month_day') {
        ...
        if (r.start) recurr.week = weekOfMonth(r.start);
    }
...
export function toEventRecurrence(
    ...
    if (r.end_type === 'date' && r.end_date) {
        ...
        const end_step = r.interval * r.end_instances;
            ...
            startOfDay(
                r.type === 'daily'
                    ? addDays(recurrence_start, end_step)
                    : r.type === 'weekly'
                      ? addWeeks(recurrence_start, end_step)
                      : addMonths(recurrence_start, end_step),
            ).valueOf() - 1;
    }
