...
@Component({
    ...
    template: `
        <div
            ...
            *ngIf="show"
        >
            <cdk-virtual-scroll-viewport itemSize="32" class="flex-1 h-1/2 w-full">
                <div
...
export class DebugConsoleComponent extends BaseClass {
    ...
    public logDisplay(log: ClientEvent) {
        return `
            <span class="text-xs p-1 bg-white/10 rounded">${format(log.timestamp,'yyyy/MM/dd h:mma')}</span>
            &nbsp;
            <span class="uppercase text-xs p-1 bg-purple-600/30 rounded">${log.type}</span>
            &nbsp;
            <span class="capitalize text-xs p-1 bg-blue-600/30 rounded w-16 text-center">${log.subtype}</span>
            &nbsp;
            ${log.data}
        `;
    }
