...
var EmergencyContactsService = class _EmergencyContactsService {
  ...
  async _queryLegacyMetadata(bld) {
    try {
      const { details } = await ju(bld.id, "emergency_contacts");
      return details || {
        contacts: [],
        roles: []
      };
  ...
  async ensureCategoryExists() {
    ...
    if (existing)
      return existing;
    try {
      const new_category = await saveAssetCategory(Ys(new or({
  ...
  async migrateFromMetadata() {
    ...
    try {
      ...
      if (legacy_data.roles?.length) {
        await saveAssetCategory(Ys(new or(__spreadProps(__spreadValues({}, category), {
          hidden: true,
  ...
  async updateRoles(roles) {
    try {
      ...
      await saveAssetCategory(new or(__spreadProps(__spreadValues({}, category), {
        description: JSON.stringify({ roles })
      })));
      this._change.set(Date.now());
      return true;
...
//# sourceMappingURL=staff.routes-R3LH7XAV.js.map
