...
var EmergencyContactsService = class _EmergencyContactsService {
  ...
  async _queryLegacyMetadata(bld) {
    try {
      const { details } = await Vu(bld.id, "emergency_contacts");
      return details || {
        contacts: [],
        roles: []
      };
  ...
  async ensureCategoryExists() {
    ...
    if (existing)
      return existing;
    try {
      const new_category = await saveAssetCategory(ni(new _r({
  ...
  async migrateFromMetadata() {
    ...
    try {
      ...
      if (legacy_data.roles?.length) {
        await saveAssetCategory(ni(new _r(__spreadProps(__spreadValues({}, category), {
          hidden: true,
  ...
  async updateRoles(roles) {
    try {
      ...
      await saveAssetCategory(new _r(__spreadProps(__spreadValues({}, category), {
        description: JSON.stringify({ roles })
      })));
      this._change.set(Date.now());
      return true;
...
//# sourceMappingURL=staff.routes-OJ7BPNWE.js.map
