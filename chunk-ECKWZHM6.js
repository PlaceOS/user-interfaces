...
var _AssetManagerStateService = class _AssetManagerStateService extends AsyncHandler {
  constructor() {
    ...
    this.settings = combineLatest([
      this._org.active_building,
      this._change
    ]).pipe(filter(([_]) => !!_), switchMap(([_]) => Pu(_.id, "assets-settings").pipe(catchError((_2) => of({})))), map((_) => _.details || {}), shareReplay(1));
  ...
  async postForm() {
    ...
    data.other_data = Ki(other_data, [void 0, null, ""]);
  ...
  async getConfig(zone_id = this._org.building.id) {
    const rules = (await Pu(zone_id, "assets_config").toPromise()).details;
  ...
  async saveSettings(settings) {
    ...
    const result = await Tu(this._org.building.id, {
...
//# sourceMappingURL=chunk-ECKWZHM6.js.map
