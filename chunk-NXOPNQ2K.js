...
var BookingFormService = class _BookingFormService extends AsyncHandler {
  ...
  async listAvailableResources() {
    ...
    await firstValueWhere(this._form_value_debounced.value, (value) => value === form2, this._injector);
  ...
  async _computeHasAssignedDesk(user_email = currentUser()?.email) {
    ...
    const desk_lists = await Promise.all(buildings.map((building) => sc(building.id, { name: "desks" }).then((data) => flatten(data.map(map_metadata))).catch(() => [])));
  ...
  async _loadBookingRulesHost(host) {
    ...
    return ja(host).catch(() => ({ email: host }));
  }
  /**
  ...
  async loadResourceList(type) {
    ...
    if (use_region) {
      ...
      const lists = await Promise.all(buildings.map((_) => sc(_.id, { name: type }).then((data2) => flatten(data2.map(map_metadata)))));
...
//# debugId=4a292e0a-28b3-5d55-869a-aa2ab5cef374
//# sourceMappingURL=chunk-NXOPNQ2K.js.map
