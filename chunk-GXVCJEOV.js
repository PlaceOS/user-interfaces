...
function saveAssetsInBulk(assets) {
  ...
  return assets.every((item) => item?.id) ? Zl(assets) : addAssetsInBulk(assets);
}
async function addAssetsInBulk(assets) {
  const response = await v(`${u()}/assets/bulk`, JSON.stringify(assets), {});
  return response.map((item) => new Sr(item));
}
function saveAssetPurchaseOrder(order) {
  return order.id ? ad(order.id, order) : hd(order);
}
function getGroupsWithAssets(query = {}) {
  return queryAssetTypes(query);
}
var _GROUP_FULL_CACHE = /* @__PURE__ */ new Map();
async function showGroupFull(id2, query = {}) {
  if (_GROUP_FULL_CACHE.has(id2))
    return _GROUP_FULL_CACHE.get(id2);
  const [type, categories, assets, purchase_orders] = await Promise.all([
    Yl(id2),
    queryAssetCategories(),
    queryAssets(__spreadProps(__spreadValues({}, query), { type_id: id2, limit: 2e3 })),
  ...
  _GROUP_FULL_CACHE.set(id2, product);
  return product;
}
...
async function removeAssetRequests(id2) {
  ...
  return Promise.all(requests.filter((_) => _.asset_id === id2 || _.asset_ids?.includes(id2)).map((request) => removeBooking(request.id)));
}
...
//# debugId=479fda8a-44c4-5bee-8b24-effd7dcd4798
//# sourceMappingURL=chunk-GXVCJEOV.js.map
