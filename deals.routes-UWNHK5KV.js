...
var DealsService = class _DealsService extends AsyncHandler {
  constructor() {
    ...
    this._deals = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_deals" } : (
      ...
      loader: async ({ params }) => {
        ...
        const metadata = await Vu(params.building, "deals-n-offers").catch(() => ({ details: [] }));
  ...
  async getDeals() {
    const metadata = await Vu(this._org.building.id, "deals-n-offers").catch(() => null);
    return metadata?.details instanceof Array ? metadata.details : [];
  }
  async saveDeal(deal) {
    const metadata = await Vu(this._org.building.id, "deals-n-offers").catch(() => null);
...
//# debugId=3fc9a9f3-127e-5dee-bcaf-e72249e99838
//# sourceMappingURL=deals.routes-UWNHK5KV.js.map
