...
var FullscreenEmbedComponent = class _FullscreenEmbedComponent {
  constructor() {
    ...
    this.embed_url = signal(this._data, ...ngDevMode ? [{ debugName: "embed_url" }] : (
...
var TeamScheduleService = class _TeamScheduleService {
  ...
  constructor() {
    ...
    this._filters = signal(__spreadValues({}, DEFAULT_FILTERS), ...ngDevMode ? [{ debugName: "_filters" }] : (
      /* istanbul ignore next */
      []
    ));
    ...
    this.select_mode = signal(false, ...ngDevMode ? [{ debugName: "select_mode" }] : (
      /* istanbul ignore next */
      []
    ));
    this._selected_member_ids = signal(/* @__PURE__ */ new Set(), ...ngDevMode ? [{ debugName: "_selected_member_ids" }] : (
      /* istanbul ignore next */
      []
    ));
    this._favorite_ids = signal(/* @__PURE__ */ new Set(), ...ngDevMode ? [{ debugName: "_favorite_ids" }] : (
      /* istanbul ignore next */
      []
    ));
    this._team_member_ids = signal(/* @__PURE__ */ new Set(), ...ngDevMode ? [{ debugName: "_team_member_ids" }] : (
      /* istanbul ignore next */
      []
    ));
    ...
    this._team_members = signal([], ...ngDevMode ? [{ debugName: "_team_members" }] : (
      /* istanbul ignore next */
      []
    ));
    ...
    this.total_members = computed(() => this._team_members().length, ...ngDevMode ? [{ debugName: "total_members" }] : (
      /* istanbul ignore next */
      []
    ));
    ...
    this.selected_count = computed(() => this._selected_member_ids().size, ...ngDevMode ? [{ debugName: "selected_count" }] : (
      /* istanbul ignore next */
      []
    ));
  ...
  async _fetchColleagues() {
    try {
      ...
      if (!user?.id)
        return [];
      const metadata = await Fu(user.id, "contacts").catch(() => ({}));
      const list = metadata?.details instanceof Array ? metadata.details : [];
      const users = await Promise.all(list.map((contact) => Ta(contact.email).catch(() => contact)));
  ...
  async _buildTeamMembers(users, favorite_ids, team_ids) {
    ...
    const bookings_promises = users.map((user) => lastValueFrom(queryBookings({
      email: user.email,
      type: "desk",
...
var AddColleaguesModalComponent = class _AddColleaguesModalComponent extends AsyncHandler {
  constructor() {
    ...
    this.search_term_signal = signal("", ...ngDevMode ? [{ debugName: "search_term_signal" }] : (
      /* istanbul ignore next */
      []
    ));
    ...
    this.loading_users = computed(() => !!this._loading_state(), ...ngDevMode ? [{ debugName: "loading_users" }] : (
      /* istanbul ignore next */
      []
    ));
    this.available_users = computed(() => this._search_results(), ...ngDevMode ? [{ debugName: "available_users" }] : (
      /* istanbul ignore next */
      []
    ));
    this.existing_contacts = computed(() => this._existing_contacts(), ...ngDevMode ? [{ debugName: "existing_contacts" }] : (
      /* istanbul ignore next */
      []
    ));
  }
...
//# sourceMappingURL=chunk-CIMONDXP.js.map
