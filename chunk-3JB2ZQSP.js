...
async function resolveCateringTypeId(caterer = "") {
  ...
  const type = types.find((_) => normalise_name(_.name) === normalise_name(type_name));
  if (type)
    return type.id;
  try {
    ...
    const type2 = types2.find((_) => normalise_name(_.name) === normalise_name(type_name));
    if (type2)
      return type2.id;
    throw error;
  }
...
//# sourceMappingURL=chunk-3JB2ZQSP.js.map
