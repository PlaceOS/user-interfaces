...
var LandingStateService = class _LandingStateService extends AsyncHandler {
  constructor() {
    ...
    this.search_fn = async (q) => this._settings.get("app.basic_user_search") || this._settings.get("app.colleagues_require_auth") !== false ? Ea({ q, authority_id: Rt()?.id }).then(({ data }) => data.map((_) => new StaffUser(_))) : searchStaff(q);
    ...
    effect((onCleanup) => {
      ...
      for (const [idx, space] of list.entries()) {
        const binding = Fl(space.id, "Bookings").variable("status");
  ...
  async updateContacts() {
    const metadata = await ju(currentUser().id, "contacts");
    const list = metadata.details instanceof Array ? metadata.details : [];
    const users = await Promise.all(list.map((_) => Ca(_.email).catch(() => _)));
...
//# sourceMappingURL=chunk-GIPZWHUJ.js.map
