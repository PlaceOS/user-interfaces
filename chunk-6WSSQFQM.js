...
var TeamScheduleService = class _TeamScheduleService {
  ...
  async _fetchColleagues() {
    try {
      ...
      if (!user?.id)
        return [];
      const metadata = await ic(user.id, "contacts").catch(() => ({}));
      const list = metadata?.details instanceof Array ? metadata.details : [];
      const users = await Promise.all(list.map((contact) => Za(contact.email).catch(() => contact)));
...
//# debugId=08cb9cf6-ef0f-5645-912d-595dfa09cfb1
//# sourceMappingURL=chunk-6WSSQFQM.js.map
