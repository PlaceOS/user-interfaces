...
function saveAsset(asset) {
  return asset.id ? kl(asset.id, asset) : Sl(asset);
}
...
async function addAssetsInBulk(assets) {
  const response = await S(`${u()}/assets/bulk`, JSON.stringify(assets), {});
  return response.map((item) => new yr(item));
}
function saveAssetPurchaseOrder(order) {
  return order.id ? zl(order.id, order) : Fl(order);
}
...
//# debugId=a4f7315e-1538-5f7f-8dea-b1849098e302
//# sourceMappingURL=chunk-PUQGM7BC.js.map
