...
var _ReportsStateService = class _ReportsStateService {
  ...
  constructor() {
    ...
    this.counts = this._options.pipe(debounceTime(500), switchMap((filters) => {
      ...
      return Promise.all(zones.map((z) => Pu(z, "desks").pipe(catchError(() => of({ details: [] })), map((m) => [z, m.details.length])).toPromise()));
    }), map((list) => {
      const map2 = {};
...
var _ContactTracingStateService = class _ContactTracingStateService {
  constructor() {
    ...
    this.events = combineLatest([
      ...
      const mod = Qa(this.system_id, "ContactTracing");
...
var _LockersReportService = class _LockersReportService {
  constructor() {
    ...
    this.counts$ = this._options.pipe(debounceTime(500), switchMap((filters) => {
      ...
      return Promise.all(zones.map((z) => Pu(z, "lockers-spaces").pipe(catchError(() => of({ details: [] })), map((m) => [z, m.details.length])).toPromise()));
    }), map((list) => {
      const mapping = {};
...
var _ParkingReportService = class _ParkingReportService {
  constructor() {
    ...
    this.counts$ = this._options.pipe(debounceTime(500), switchMap((filters) => {
      ...
      return Promise.all(zones.map((z) => Pu(z, "parking-spaces").pipe(catchError(() => of({ details: [] })), map((m) => [z, m.details.length])).toPromise()));
    }), map((list) => {
      const mapping = {};
      list.forEach(([id, count]) => mapping[id] = Math.max(count || 0, 1));
      return mapping;
...
//# sourceMappingURL=reports.module-EZZ5ZGDD.js.map
