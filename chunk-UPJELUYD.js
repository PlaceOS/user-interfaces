...
var _BookingFormService = class _BookingFormService extends AsyncHandler {
  ...
  newForm(type, booking = new Booking({})) {
    ...
    this.form.reset();
    this.form.patchValue(as(__spreadValues(__spreadValues({}, booking.extension_data), booking), [null, void 0, ""]));
  ...
  constructor() {
    ...
    this.booking_rules = combineLatest([this._org.building_list, this._options]).pipe(switchMap(([list, { type }]) => Promise.all(list.map((bld) => lastValueFrom(Xu(bld.id, `${type}_booking_rules`))))), map((building_rules) => {
      const mapping = {};
  ...
  resetForm() {
    ...
    this.form.patchValue(as(__spreadValues(__spreadValues({}, booking || {}), booking?.extension_data || {}), [null, void 0, ""]));
    this._options.next({ type: this._options.getValue().type });
  }
  clearForm() {
  ...
  storeForm() {
    sessionStorage.setItem("PLACEOS.booking_form", JSON.stringify(__spreadValues(__spreadValues({}, this._booking.getValue()), as(this.form.getRawValue() || {}, [
      null,
      void 0,
      ""
    ]))));
  ...
  loadForm() {
    ...
    const booking_data = as(__spreadValues(__spreadValues(__spreadValues({}, data), booking || {}), booking?.extension_data || {}), [null, void 0, ""]);
  ...
  async _checkResourceRules(assets, date, duration, host) {
    ...
    const user = current_user.email === host ? current_user : await lastValueFrom(Aa(host)).catch(() => ({
      email: host
    }));
    if (!assets?.length)
      return true;
  ...
  loadResourceList(type) {
    ...
    if (use_region) {
      ...
      return forkJoin(buildings.map((_) => rc(_.id, { name: type }).pipe(map((data) => flatten(data.map(map_metadata)))))).pipe(map((_) => flatten(_)));
    }
    return rc(id, {
      name: type
...
var _ParkingService = class _ParkingService extends AsyncHandler {
  constructor() {
    ...
    this.spaces = combineLatest([this.levels]).pipe(filter(([lvls]) => !!lvls[0]?.id), switchMap(([levels]) => {
      ...
      return forkJoin(levels.map((lvl) => Xu(lvl.id, "parking-spaces").pipe(map((d) => (d.details instanceof Array ? d.details : []).map((s) => __spreadProps(__spreadValues({}, s), {
        zone_id: lvl.id
      }))))));
...
//# sourceMappingURL=chunk-UPJELUYD.js.map
