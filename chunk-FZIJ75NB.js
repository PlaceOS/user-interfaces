...
var _EmergencyContactModalComponent = class _EmergencyContactModalComponent {
  constructor() {
    ...
    this.data = combineLatest([
      ...
      this._changes
    ]).pipe(filter(([bld]) => !!bld), switchMap(([bld]) => Iu(bld.id, "emergency_contacts")), map(({ details }) => details || { roles: [], contacts: [] }), shareReplay(1));
  ...
  async addRole() {
    ...
    await Pu(this._org.building.id, {
...
var _RoleManagementModalComponent = class _RoleManagementModalComponent {
  constructor() {
    ...
    this.data = combineLatest([
      this._org.active_building,
      this._changes
    ]).pipe(filter(([bld]) => !!bld), switchMap(([bld]) => Iu(bld.id, "emergency_contacts")), map(({ details }) => {
...
//# sourceMappingURL=chunk-FZIJ75NB.js.map
