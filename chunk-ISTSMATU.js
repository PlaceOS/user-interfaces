...
var ParkingStateService = class _ParkingStateService extends AsyncHandler {
  ...
  isWaitlisted(booking) {
    ...
    const now = Date.now();
    const week_start = this._settings.get("app.parking.waitlist_week_start") || {};
    ...
    let current_week_start = set(startOfWeek(zoned_now, { weekStartsOn: week_start_day }), {
    ...
    const current_week_start_time = timezone ? fromZonedTime(current_week_start, timezone).valueOf() : current_week_start.valueOf();
    const next_week_start_time = timezone ? fromZonedTime(next_week_start, timezone).valueOf() : next_week_start.valueOf();
    return booking.date >= current_week_start_time && booking.date < next_week_start_time;
  }
...
//# sourceMappingURL=chunk-ISTSMATU.js.map
