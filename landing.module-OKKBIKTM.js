...
var _LandingStateService = class _LandingStateService extends AsyncHandler {
  constructor() {
    ...
    this._booking_rules = this._org.active_building.pipe(filter((bld) => !!bld), switchMap((bld) => nc(bld.id, `room_booking_rules`).pipe(catchError(() => of({ details: [] })))), map((_) => _?.details instanceof Array ? _.details : []), shareReplay(1));
    ...
    this._space_statuses = this._filtered_spaces.pipe(tap((_) => this.unsubWith("bind:")), switchMap((list) => combineLatest((list || []).map((_) => {
      const binding = wh(_.id, "Bookings").variable("status");
    ...
    this.search_fn = (q) => this._settings.get("app.basic_user_search") || this._settings.get("app.colleagues_require_auth") !== false ? Oa({ q, authority_id: Ut()?.id }).pipe(map(({ data }) => data.map((_) => new StaffUser(_)))) : searchStaff(q);
  ...
  async updateContacts() {
    const metadata = await lastValueFrom(nc(currentUser().id, "contacts"));
    const list = metadata.details instanceof Array ? metadata.details : [];
    const users = await Promise.all(list.map((_) => lastValueFrom(Ea(_.email).pipe(catchError(() => of(_))))));
  ...
  async addContact(user) {
    ...
    await lastValueFrom(rc(currentUser().id, {
      name: "contacts",
...
var _LandingFavouritesComponent = class _LandingFavouritesComponent extends AsyncHandler {
  ...
  async ngOnInit() {
    this._room_alerts = await lastValueFrom(nc(this._org.organisation.id, "room_alerts").pipe(map((v) => v.details)));
  }
...
//# sourceMappingURL=landing.module-OKKBIKTM.js.map
