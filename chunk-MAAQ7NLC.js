...
var BookingRulesFormComponent = class _BookingRulesFormComponent {
  constructor() {
    ...
    this.building_zones = toObservable(this._org.active_building).pipe(filter((_) => !!_), switchMap((bld) => ah({ parent_id: bld.id }).catch(() => ({ data: [] }))), map((res) => res.data), shareReplay(1));
...
var BookingRulesModalComponent = class _BookingRulesModalComponent {
  constructor() {
    ...
    this._booking_rules = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_booking_rules" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const { details } = await rc(params.building, `${this._data.type}_booking_rules`).catch(() => ({ details: [] }));
  ...
  async removeRuleset(ruleset) {
    ...
    if (index >= 0) {
      ...
      await oc(this._org.building.id, {
...
//# sourceMappingURL=chunk-MAAQ7NLC.js.map
