...
var AppSettingsModalComponent = class _AppSettingsModalComponent {
  ...
  async ngOnInit() {
    ...
    const zone_settings = (await Vu(this.zone.id, `${this.workplace_key}`).catch(() => ({ details: {} }))).details;
    const parent_settings = (await Vu(this.zone.parent_id, `${this.workplace_key}`).catch(() => ({ details: {} }))).details;
    const org_settings = (await Vu(this._org.organisation.id, `${this.workplace_key}`).catch(() => ({ details: {} }))).details;
...
var AutoReleaseSettingsModalComponent = class _AutoReleaseSettingsModalComponent {
  ...
  async loadSettings(id) {
    ...
    const settings = (await ia({ parent_id: id })).data;
    const unencrypted = settings.find((_) => _.encryption_level === ze.None);
    if (!unencrypted)
      return;
    try {
  ...
  async save() {
    ...
    const settings = (await ia({ parent_id: this.id })).data;
    let unencrypted = settings.find((_) => _.encryption_level === ze.None);
    if (!unencrypted) {
      unencrypted = new Pe({
        parent_id: this.id,
        encryption_level: ze.None,
        settings_string: ""
      });
    }
    ...
    unencrypted.id ? await oa(unencrypted.id, unencrypted).catch(on_error) : await ua(unencrypted).catch(on_error);
    const metadata_key = this._settings.get("app.workplace_metadata_key") || "workplace_app";
    const metadata = await Vu(this.id, metadata_key);
...
var BuildingFormComponent = class _BuildingFormComponent extends AsyncHandler {
  ...
  async saveChanges() {
    ...
    const body = __spreadProps(__spreadValues({}, data), {
      tags: ["building"],
      name: `BLD ${It().description} ${data.display_name}`
    });
    const building = await (data.id ? sh(data.id, body) : ih(body)).catch((e) => {
...
var InductionSettingsModalComponent = class _InductionSettingsModalComponent {
  ...
  async loadSettings() {
    ...
    const [bld_metadata, org_metadata, org_settings] = await Promise.all([
      Vu(this._zone_id, visitor_kiosk_app),
      Vu(this._org.organisation.id, visitor_kiosk_app),
      Vu(this._org.organisation.id, "settings")
    ]);
  ...
  async save() {
    ...
    this._dialog_ref.disableClose = true;
    const metadata = await Vu(this._zone_id, visitor_kiosk_app);
    const con_metadata = await Vu(this._zone_id, concierge_app);
    ...
    const result_visitor = await Xu(this._zone_id, {
    ...
    const result_concierge = await Xu(this._zone_id, {
...
var ItemListModalComponent = class _ItemListModalComponent {
  ...
  async ngOnInit() {
    const metadata_key = this._settings.get("app.workplace_metadata_key") || "workplace_app";
    const metadata = await Vu(this._bld_id, metadata_key);
  ...
  async save() {
    ...
    let resp = await Xu(this._bld_id, {
      name: metadata_key,
    ...
    const concierge_metadata = await Vu(this._bld_id, metadata_key);
    concierge_metadata.details.support_issue_types = items;
    resp = await Xu(this._bld_id, {
      name: concierge_key,
...
var BuildingManagementService = class _BuildingManagementService {
  ...
  editBuildingMetadata(zone = new Yt()) {
...
var BookingPanelSettingsModalComponent = class _BookingPanelSettingsModalComponent extends AsyncHandler {
  ...
  async ngOnInit() {
    ...
    let settings = [];
    try {
      const response = await ia({ parent_id: this.zone.id });
      settings = response?.data || [];
    } catch {
    }
    const unencrypted_settings = settings.find((block) => block.encryption_level === ze.None);
  ...
  async save() {
    ...
    let unencrypted_settings = settings.find((block) => block.encryption_level === ze.None);
    if (!unencrypted_settings)
    ...
    const update = unencrypted_settings.id ? oa(unencrypted_settings.id, new_setting) : ua(new_setting);
    try {
      await update;
    } catch (e) {
      this._dialog_ref.disableClose = false;
...
var ConciergeSettingsFormModalComponent = class _ConciergeSettingsFormModalComponent {
  ...
  async _getMetadata(id) {
    const metadata = await Vu(id, this.settings_key);
...
var LevelMapEditorModalComponent = class _LevelMapEditorModalComponent {
  ...
  async save() {
    ...
    try {
      ...
      const level_name = this._level.name || `LEVEL ${It().description} ${this._level.display_name || this._level.id}`;
      const response = await sh(this._level.id, {
        id: this._level.id,
...
var LevelModalComponent = class _LevelModalComponent {
  ...
  async save() {
    ...
    const resp = await (data.id ? sh(data.id, __spreadProps(__spreadValues({}, data), {
      name: `LEVEL ${It().description} ${data.display_name}`
    })) : ih(__spreadProps(__spreadValues({}, data), {
      name: `LEVEL ${It().description} ${data.display_name}`
    }))).catch();
    if (resp.id)
...
var RegionModalComponent = class _RegionModalComponent extends AsyncHandler {
  ...
  async save() {
    ...
    this.loading.set(true);
    const resp = await (data.id ? sh(data.id, __spreadProps(__spreadValues({}, data), {
      name: `REGION ${It().description} ${data.display_name}`
    })) : ih(__spreadProps(__spreadValues({}, data), {
      name: `REGION ${It().description} ${data.display_name}`
    }))).catch();
    if (resp.id)
...
var RegionManagementService = class _RegionManagementService {
  ...
  editRegionMetadata(region = new Yt()) {
    const ref = this._dialog.open(AppSettingsModalComponent, {
      data: { zone: region }
    });
    ref.afterClosed().subscribe((data) => {
...
//# sourceMappingURL=zone-manager.routes-ZSCRKXAP.js.map
