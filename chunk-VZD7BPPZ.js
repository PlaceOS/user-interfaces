...
function saveAsset(asset) {
  return asset.id ? jl(asset.id, asset) : Gl(asset);
}
function saveAssetsInBulk(assets) {
  if (!assets?.length)
    return Promise.resolve([]);
  return assets.every((item) => item?.id) ? Ql(assets) : addAssetsInBulk(assets);
}
...
function saveAssetPurchaseOrder(order) {
  return order.id ? ud(order.id, order) : cd(order);
}
...
//# debugId=f4de56ad-ee7f-5d4f-bd8e-ad9ea14a68b3
//# sourceMappingURL=chunk-VZD7BPPZ.js.map
