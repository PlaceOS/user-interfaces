...
@Component({
    ...
    template: `
        <div class="h-full w-full bg-black relative text-white">
            ...
            <div qr-checkin class="absolute top-4 right-4 text-xl w-40 space-y-4">
                <img class="w-full" [src]="qr_code" />
                <div class="w-full text-lg">
                    Scan QR code to book this room or view details
                </div>
            </div>
            <div class="absolute top-2/3 left-1/2 -translate-x-1/2 text-3xl font-light">
...
export class PanelViewDetailsComponent {
    ...
    public ngOnInit() {
        const url = `${location.origin}${location.pathname}/#/details/${this._state.system}`;
        console.log('URL:', url);
