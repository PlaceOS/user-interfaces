...
test.describe('US-CONFIG-001 & US-CONFIG-002: Configure Event Types and Booking Rules', () => {
    test('should display booking rules button on menu view', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });

        // Should show booking rules button with menu_book icon
        const rules_button = page.locator('button').filter({ has: page.locator('icon:has-text("menu_book")') });
        await expect(rules_button).toBeVisible({ timeout: ACTION_TIMEOUT });
    });

    test('should open booking rules modal when button clicked', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
    ...
    test('should show configuration options in booking rules modal', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
        ...
        const elements = dialog.locator('button, input, mat-select, mat-checkbox');
...
test.describe('US-CONFIG-003 & US-CONFIG-004: Room Management', () => {
    test('should display room availability button on menu view', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });

        // Should show room availability button with event_available icon
        const room_button = page.locator('button').filter({ has: page.locator('icon:has-text("event_available")') });
        await expect(room_button).toBeVisible({ timeout: ACTION_TIMEOUT });
    });

    test('should open room availability modal when button clicked', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
...
test.describe('US-CONFIG-005 to US-CONFIG-008: Charge Codes Management', () => {
    test('should display charge codes button on menu view', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });

        // Should show charge codes button with payments icon
        const codes_button = page.locator('button').filter({ has: page.locator('icon:has-text("payments")') });
        await expect(codes_button).toBeVisible({ timeout: ACTION_TIMEOUT });
    });

    test('should open charge codes modal when button clicked', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
    ...
    test('should have interactive elements in charge codes modal', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
...
test.describe('US-CONFIG-009: Require Order Notes', () => {
    test('should have notes requirement option in booking rules', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
...
test.describe('Configuration - Modal Interactions', () => {
    test('should close booking rules modal with close button', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
        ...
        const close_button = dialog.locator('button').filter({ has: page.locator('icon:has-text("close")') });
    ...
    test('should close room availability modal with close button', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
    ...
    test('should close charge codes modal with close button', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
...
test.describe('Configuration - Button Tooltips', () => {
    ...
    test('should show tooltip on booking rules button hover', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
    ...
    test('should show tooltip on room availability button hover', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
    ...
    test('should show tooltip on charge codes button hover', async ({ page }) => {
        await page.goto(MENU_URL);
        await page.locator('catering-topbar').waitFor({ timeout: LOAD_TIMEOUT });
