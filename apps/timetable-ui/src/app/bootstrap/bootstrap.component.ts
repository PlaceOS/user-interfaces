...
const STORE_KEY = 'PLACEOS.TIMETABLE_UI.system';

@Component({
    selector: '[app-bootstrap]',
    template: `
        <div
            class="center rounded shadow w-64 bg-white text-center flex flex-col items-center p-4"
        >
            <h2 class="py-2 px-4 m-0 w-full text-xl">Timetable UI Setup</h2>
            <ng-container *ngIf="!loading; else load_state">
                <p class="description py-4">
                    Please select a system from the dropdown below:
                </p>
...
export class BootstrapComponent extends BaseClass implements OnInit {
    ...
    private checkBootstrapped(): void {
        ...
        if (localStorage) {
            ...
            if (system_id) {
                this._router.navigate([system_id]);
                return;
            }
        }
    ...
    /**
     * Return a filtered list based on the search string
