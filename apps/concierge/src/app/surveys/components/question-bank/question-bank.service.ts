...
export class QuestionBankService extends BaseClass {
    ...
    private async loadQuestions() {
        ...
        const q = (await queryQuestions()
            .pipe(
                ...
                map(
                    (res: SurveyQuestion[]) =>
                        res?.map((e) => this.translateToQuestion(e)) || []
                ),
                catchError((err) =>
    ...
    private async saveQuestion(question: Question) {
        ...
        const res: Question = (await addQuestion(this.translateToSurveyQuestion(question))
            .pipe(
                first(),
                map((res) => this.translateToQuestion(res)),
                catchError((err) =>
    ...
    private translateToSurveyQuestion(q: Question) {
    ...
    private translateToQuestion(q: SurveyQuestion) {
        return {
            id: q.id,
            title: q.title,
