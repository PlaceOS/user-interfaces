...
import { distinctUntilKeyChanged, filter, shareReplay, take, tap } from "rxjs/operators";
...
@Component({
    ...
    template: `
        ...
        <div class="flex flex-row items-center bg-white dark:bg-neutral-700 h-20 px-4 w-full dark:border-neutral-500">
            <div class="flex space-x-2">
                <mat-form-field appearance="outline" [subscriptSizing]="'dynamic'">
                    ...
                    >
                        <mat-option *ngFor="let b of buildings$ | async" [value]="b.id">{{b.display_name || b.name}}</mat-option>
                    </mat-select>
                </mat-form-field>
                ...
                <mat-form-field appearance="outline" [subscriptSizing]="'dynamic'">
                    ...
                    >
                        <mat-option *ngFor="let op of triggerOptions" [value]="op.value">
                            {{op.name}}
                        </mat-option>
                    </mat-select>
                </mat-form-field>
...
export class SurveyCreatorTopbarComponent extends BaseClass implements OnInit{
    ...
    private doBuildingChange(building_id:string){
        if(!building_id?.length) return;
        this.selected_building = this._org.buildings.find(e => e.id === building_id);
