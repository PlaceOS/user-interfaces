...
export interface CateringMenuItem {
    ...
    unit: 'per_person' | 'each' | 'per_event' | 'per_dozen' | 'per_gallon' | 'per_quart' | 'per_platter' | 'per_bowl' | 'per_hour' | 'per_bag';
    category: string;
    subcategory: string;
}
...
// ── Catering Tiers ───────────────────────────────────────────────────
// full_service  = UCLA provides on-site catering (all menus available)
// coordination  = Catering available via coordination (reception, beverages, snacks, setup, AV)
// self_service  = No catering; only AV & setup items
export type CateringTier = 'full_service' | 'coordination' | 'self_service';

export const CATERING_TIER_OPTIONS = [
    { value: 'full_service' as CateringTier, label: 'Full Service', description: 'On-site catering with full UCLA menus' },
    { value: 'coordination' as CateringTier, label: 'Coordination', description: 'Catering arranged through external providers' },
    { value: 'self_service' as CateringTier, label: 'Self-Service / None', description: 'No catering — AV & setup only' },
];

// Which menu categories are visible at each catering tier (food only)
// AV, Setup & Service Charges have their own sections in the venue form
export const TIER_CATEGORIES: Record<CateringTier, string[]> = {
    full_service: [
        'breakfast', 'brunch', 'lunch', 'dinner', 'breaks',
        'reception', 'beverages', 'bakery', 'gameday',
    ],
    coordination: [
        'reception', 'beverages', 'bakery',
    ],
    self_service: [],
};

// Categories that belong to AV & Technology section
export const AV_CATEGORIES = ['av_rental', 'av_staffing'];

// Categories that belong to Setup & Furniture section
export const SETUP_CATEGORIES = ['setup'];

// Default catering tier per building (based on UCLA venue services data)
export const BUILDING_CATERING_TIER: Record<string, CateringTier> = {
    'bld-01': 'full_service',   // Covel Commons — Full Catering Services
    'bld-02': 'full_service',   // Carnesale Commons — Full Catering Services
    'bld-03': 'full_service',   // De Neve Plaza — Full Catering Services
    'bld-04': 'full_service',   // Kerckhoff Hall — Full-Service Catering
    'bld-05': 'full_service',   // Luskin Conference Center — On-Site Event Catering
    'bld-06': 'full_service',   // Ackerman Union — Full-Service Catering (ASUCLA)
    'bld-07': 'coordination',   // Sunset Canyon Recreation — Catering Coordination
    'bld-08': 'coordination',   // Court of Sciences / Haines Hall — Catering Coordination
    'bld-09': 'coordination',   // Murphy Hall — Catering Coordination
    'bld-10': 'coordination',   // Pauley Pavilion — Catering Coordination
    'bld-11': 'coordination',   // Hammer Museum — Catering Coordination
    'bld-12': 'coordination',   // Schoenberg Music — Catering Coordination
    'bld-13': 'coordination',   // Theater, Film & Television — Catering Coordination
    'bld-14': 'coordination',   // CNSI — Catering Coordination
    'bld-15': 'coordination',   // Botanical Gardens — Catering Coordination
    'bld-16': 'coordination',   // Samueli Engineering — Catering Coordination
    'bld-17': 'coordination',   // Law School — Catering Coordination
    'bld-18': 'coordination',   // Moore Hall — Catering Coordination
    'bld-19': 'coordination',   // Luskin School of Public Affairs — Catering Coordination
    'bld-20': 'self_service',   // Powell Library / YRL — No catering
    'bld-21': 'coordination',   // DataX — Catering Coordination
    'bld-22': 'coordination',   // Kaplan Hall — Catering Support
    'bld-23': 'full_service',   // Lake Arrowhead Lodge — Award-Winning Dining
    'bld-24': 'coordination',   // UCLA Recreation — Catering Coordination
    'bld-25': 'coordination',   // Campus Outdoor Spaces — Catering Coordination
};

// ══════════════════════════════════════════════════════════════════════
// MENU PACKAGES (from Simple Menus PDF)
// ══════════════════════════════════════════════════════════════════════

export const UCLA_CATERING_MENU: CateringCategory[] = [
    // ── Breakfast ─────────────────────────────────────────────────────
    {
        id: 'breakfast',
        label: 'Breakfast',
    ...
    // ── Breaks & Snacks ───────────────────────────────────────────────
    {
        ...
        items: [
            ...
            { id: 'ee_am_break', name: 'EE AM Break w/Coffee (A–F)', default_price: 18.50, unit: 'per_person', category: 'breaks', subcategory: 'Conference' },
            { id: 'ee_am_continental', name: 'EE AM Break w/Continental & Coffee (A–F)', default_price: 25.50, unit: 'per_person', category: 'breaks', subcategory: 'Conference' },
    ...
    // ── Reception & Hors d'oeuvres ────────────────────────────────────
    {
        ...
        items: [
            ...
            { id: 'american_caviar', name: 'American Caviar, Roasted Potato, Crème Fraiche', default_price: 5, unit: 'each', category: 'reception', subcategory: "Hors d'oeuvres" },
            { id: 'artisan_cheese_tart', name: 'Artisan Cheese Tart, Roasted Fruit, Thyme', default_price: 3.50, unit: 'each', category: 'reception', subcategory: "Hors d'oeuvres" },
    ...
    // ── Beverages ─────────────────────────────────────────────────────
    {
        ...
        items: [
            ...
            { id: 'canned_juices', name: 'Assorted Canned Fruit Juices & V8', default_price: 2.50, unit: 'each', category: 'beverages', subcategory: 'Juice' },
...
export const ALL_MENU_ITEMS: CateringMenuItem[] = UCLA_CATERING_MENU.flatMap(
    (cat) => cat.items,
);

// ── Helper: get item by id ───────────────────────────────────────────
export const getMenuItemById = (id: string): CateringMenuItem | undefined =>
    ALL_MENU_ITEMS.find((item) => item.id === id);
