...
/** Time before the connecting screen offers recovery actions */
const SLOW_CONNECT_MS = 30 * 1000;

/**
 * Full screen shown while the panel waits for the system to connect.
 * After a delay it offers to retry, and to change the room on non-fixed
 * devices.
 */
@Component({
    selector: 'control-connecting',
    template: `
        <div
            name="loader"
    ...
    imports: [MatProgressSpinnerModule, MatRippleModule, TranslatePipe],
})
export class ControlConnectingComponent extends AsyncHandler {
    private _state = inject(ControlStateService);

    public readonly id = this._state.system_id;
    public readonly can_change_room = this._state.canChangeRoom();
    /** Whether the connection is taking longer than expected */
    public readonly slow = signal(false);

    public readonly retry = () => window.location.reload();
    public readonly changeRoom = () => this._state.changeRoom();
