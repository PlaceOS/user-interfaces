...
@Component({
    ...
    template: `
        @if (item()) {
            <div class="m-2 rounded-sm bg-base-100 p-4 text-black shadow-sm">
                <div
                    view
                    matRipple
                    class="relative mb-2 flex h-48 flex-col items-center justify-center space-y-2 rounded-sm border border-base-200"
                    [class.opacity-60]="!(input | async)"
                    [class.bg-base-200]="!(input | async)"
                    (click)="switchSource()"
                >
                    <div
                        class="absolute left-1 top-1 rounded-sm bg-secondary px-2 py-1 text-white shadow-sm"
                    >
                        {{ item()?.name }}
                    </div>
                    <icon class="text-7xl">{{
