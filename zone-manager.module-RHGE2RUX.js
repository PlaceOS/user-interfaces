...
var _AppSettingsModalComponent = class _AppSettingsModalComponent {
  ...
  async ngOnInit() {
    ...
    const zone_settings = (await Iu(this.zone.id, `${this.workplace_key}`).toPromise().catch(() => ({ details: {} }))).details;
    const parent_settings = (await Iu(this.zone.parent_id, `${this.workplace_key}`).toPromise().catch(() => ({ details: {} }))).details;
    const org_settings = (await Iu(this._org.organisation.id, `${this.workplace_key}`).toPromise().catch(() => ({ details: {} }))).details;
...
var _AutoReleaseSettingsModalComponent = class _AutoReleaseSettingsModalComponent {
  ...
  async loadSettings(id) {
    ...
    this.settings = { custom: [] };
    const settings = await nextValueFrom(yc({ parent_id: id }).pipe(map((_) => _.data)));
  ...
  async save() {
    ...
    const settings = await lastValueFrom(yc({ parent_id: this.id }).pipe(map((_) => _.data)));
    ...
    const metadata = await lastValueFrom(Iu(this.id, metadata_key));
    const details = metadata.details || {};
    details.auto_release = new_settings;
    await nextValueFrom(Pu(this.id, {
      name: metadata_key,
      details,
      description: ""
    })).catch(on_error);
...
var _BuildingFormComponent = class _BuildingFormComponent extends AsyncHandler {
  ...
  async saveChanges() {
    ...
    const building = await (data.id ? ra(data.id, body) : ia(body)).toPromise().catch((e) => {
...
var _InductionSettingsModalComponent = class _InductionSettingsModalComponent {
  ...
  async loadSettings() {
    ...
    const [bld_metadata, org_metadata, org_settings] = await Promise.all([
      await lastValueFrom(Iu(this._zone_id, visitor_kiosk_app)),
      await lastValueFrom(Iu(this._org.organisation.id, visitor_kiosk_app)),
      await lastValueFrom(Iu(this._org.organisation.id, "settings"))
    ]);
  ...
  async save() {
    ...
    this._dialog_ref.disableClose = true;
    const metadata = await lastValueFrom(Iu(this._zone_id, visitor_kiosk_app));
    const con_metadata = await lastValueFrom(Iu(this._zone_id, concierge_app));
    ...
    const result_visitor = await lastValueFrom(Pu(this._zone_id, {
    ...
    const result_concierge = await lastValueFrom(Pu(this._zone_id, {
...
var _ItemListModalComponent = class _ItemListModalComponent {
  ...
  async ngOnInit() {
    const metadata_key = this._settings.get("app.workplace_metadata_key") || "workplace_app";
    const metadata = await Iu(this._bld_id, metadata_key).toPromise();
  ...
  async save() {
    ...
    let resp = await Pu(this._bld_id, {
      name: metadata_key,
    ...
    const concierge_metadata = await Iu(this._bld_id, metadata_key).toPromise();
    concierge_metadata.details.support_issue_types = items;
    resp = await Pu(this._bld_id, {
      name: concierge_key,
...
var _BuildingManagementService = class _BuildingManagementService {
  ...
  async removeBuilding(building) {
    ...
    await oa(building.id).toPromise().catch((e) => {
...
var _BookingPanelSettingsModalComponent = class _BookingPanelSettingsModalComponent extends AsyncHandler {
  ...
  async ngOnInit() {
    ...
    const settings = await lastValueFrom(yc({ parent_id: this.zone.id }).pipe(catchError(() => of({ data: [] })), map((_) => _.data)));
...
var _ConciergeSettingsFormModalComponent = class _ConciergeSettingsFormModalComponent {
  ...
  async save() {
    ...
    await lastValueFrom(Pu(zone.id, {
  ...
  _getMetadata(id) {
    return lastValueFrom(Iu(id, this.settings_key).pipe(map((m) => m.details)));
  }
};
...
var _LevelModalComponent = class _LevelModalComponent {
  ...
  async save() {
    ...
    const resp = await (data.id ? ra(data.id, __spreadProps(__spreadValues({}, data), {
      name: `LEVEL ${kt().description} ${data.display_name}`
    })) : ia(__spreadProps(__spreadValues({}, data), {
      name: `LEVEL ${kt().description} ${data.display_name}`
    }))).toPromise().catch();
    if (resp.id)
...
var _LevelManagementService = class _LevelManagementService {
  ...
  async removeLevel(level) {
    ...
    await oa(level.id).toPromise().catch((e) => {
...
var _RegionManagementService = class _RegionManagementService {
  ...
  async removeRegion(region) {
    ...
    await oa(region.id).toPromise().catch((e) => {
...
//# sourceMappingURL=zone-manager.module-RHGE2RUX.js.map
