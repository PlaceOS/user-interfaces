...
@Component({
    ...
    template: `
        <div hidden>
            <i binding [(model)]="name" [sys]="system" mod="System" bind="name"></i>
            <i binding [(model)]="connected" [sys]="system" mod="System" bind="connected"></i>
        </div>
        <ng-container *ngIf="true; else load_state">
            <div *ngIf="connected; else power_off_state" class="absolute inset-0">
        ...
        <ng-template #power_off_state>
            <div
                ...
                (touchend)="powerOn()"
            >
                <h2 class="font-normal">Touch to Start</h2>
                <p>{{ name }}</p>
            </div>
        </ng-template>
        <ng-template #load_state>
            ...
            >
                ...
                <div class="text-2xl my-4">
                    Connecting to system(<em>{{ system }}</em
                    >)...
                </div>
...
export class ControlMainViewComponent extends BaseClass {
    ...
    public readonly powerOn = async () =>
        await getModule(this.system, 'System')?.execute('powerup', []);
    ...
    public ngOnInit(): void {
        this.subscription(
            ...
            this._route.paramMap.subscribe((params) =>
                params.has('system') ? (this.system = params.get('system')) : ''
            )
        );
        this.timeout('init', () => (!this.system ? this._router.navigate(['/bootstap']) : ''));
    }
}
