...
var TeamScheduleService = class _TeamScheduleService {
  ...
  async _fetchColleagues() {
    try {
      ...
      if (!user?.id)
        return [];
      const metadata = await Wu(user.id, "contacts").catch(() => ({}));
      const list = metadata?.details instanceof Array ? metadata.details : [];
      const users = await Promise.all(list.map((contact) => Na(contact.email).catch(() => contact)));
...
//# sourceMappingURL=chunk-DAYE4BM6.js.map
