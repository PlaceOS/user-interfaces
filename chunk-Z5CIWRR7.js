import {
  ...
  DragDropModule,
  moveItemInArray
} from "./chunk-G7AQVIJX.js";
import {
  ItemListFieldComponent,
  uniqueChipItems
} from "./chunk-UWH6XT6Z.js";
import {
...
var BookingRulesFormComponent = class _BookingRulesFormComponent {
  constructor() {
    ...
    this.building_zones = toObservable(this._org.active_building).pipe(filter((_) => !!_), switchMap((bld) => th({ parent_id: bld.id }).catch(() => ({ data: [] }))), map((res) => res.data), shareReplay(1));
...
var BookingRulesModalComponent = class _BookingRulesModalComponent {
  constructor() {
    ...
    this._booking_rules = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_booking_rules" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const { details } = await Vu(params.building, `${this._data.type}_booking_rules`).catch(() => ({ details: [] }));
  ...
  async removeRuleset(ruleset) {
    ...
    if (index >= 0) {
      ...
      await Xu(this._org.building.id, {
...
//# debugId=f151b957-91c9-56d4-b9ee-eca06fc187b6
//# sourceMappingURL=chunk-Z5CIWRR7.js.map
