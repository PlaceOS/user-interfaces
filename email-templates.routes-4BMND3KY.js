...
var EmailTemplatesStateService = class _EmailTemplatesStateService extends AsyncHandler {
  constructor() {
    ...
    this._template_definitions = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_template_definitions" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const metadata = await ju(this._org.organisation.id, "email_template_fields").catch(() => null);
  ...
  async _queryTemplates(building, region) {
    ...
    const [org_templates, bld_templates, region_templates] = await Promise.all([
      ju(this._org.organisation.id, "email_templates").then((_) => this._processTemplates(_, this._org.organisation.id)).catch(() => []),
      ju(building, "email_templates").then((_) => this._processTemplates(_, building)).catch(() => []),
      region ? ju(region, "email_templates").then((_) => this._processTemplates(_, region)).catch(() => []) : Promise.resolve([])
    ]);
  ...
  async saveTemplate(template, old_zone = "") {
    ...
    if (template.id && old_zone) {
      const old_metadata = await ju(old_zone, "email_templates");
    ...
    const metadata = await ju(template.zone_id, "email_templates");
...
//# sourceMappingURL=email-templates.routes-4BMND3KY.js.map
