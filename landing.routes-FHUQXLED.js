import {
  ...
  EventCardComponent
} from "./chunk-WSGCPWZW.js";
import "./chunk-NG2TMR75.js";
import {
  FAV_DESK_KEY
} from "./chunk-7HYVSGEI.js";
import {
  FAV_PARKING_KEY
} from "./chunk-XTEURTQ4.js";
import "./chunk-AGVI3NRE.js";
import "./chunk-PGSR5XW3.js";
import "./chunk-5XCNLPDN.js";
import "./chunk-X3YXIG2F.js";
import "./chunk-5V4URE3M.js";
import {
  FAV_LOCKER_KEY
} from "./chunk-5XMWKSKG.js";
import "./chunk-CSDLZNJ4.js";
import {
  LevelPipe
...
import "./chunk-NVLJXPRB.js";
import "./chunk-2EIM7BFT.js";
import "./chunk-CY54FZTH.js";
import {
  EventFormService
} from "./chunk-5OEIUPDZ.js";
...
var LandingStateService = class _LandingStateService extends AsyncHandler {
  constructor() {
    ...
    this.search_fn = async (q) => this._settings.get("app.basic_user_search") || this._settings.get("app.colleagues_require_auth") !== false ? Ka({ q, authority_id: Mt()?.id }).then(({ data }) => data.map((_) => new StaffUser(_))) : searchStaff(q);
    ...
    effect((onCleanup) => {
      ...
      for (const [idx, space] of list.entries()) {
        const binding = Dd(space.id, "Bookings").variable("status");
    ...
    effect(() => {
      this._upcoming_events.set(this._schedule.filtered_bookings().filter((i) => i.status !== "cancelled" && i.type !== "cancelled" && i.state !== "done" && isSameDay(i.date, Date.now())));
    });
    this.init();
  }
  async init() {
  ...
  async updateContacts() {
    const metadata = await ic(currentUser().id, "contacts");
    const list = metadata.details instanceof Array ? metadata.details : [];
    const users = await Promise.all(list.map((_) => Za(_.email).catch(() => _)));
...
var LandingFavouritesComponent = class _LandingFavouritesComponent extends AsyncHandler {
  ...
  async ngOnInit() {
    const metadata = await ic(this._org.organisation.id, "room_alerts");
...
var LandingQuickBookComponent = class _LandingQuickBookComponent {
  ...
  async book(type) {
    if (this.loading() || type === "space")
      return;
    this.loading.set(type);
    try {
      this._book_form.newForm(type);
      this._book_form.setOptions({ type });
      const resources = await firstValueFrom(from(this._book_form.listAvailableResources()).pipe(timeout(15e3)));
      if (!resources.length) {
      ...
      notifyError(`Unable to book a ${type}. Please try again or use Bookings.`);
    } finally {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(LandingQuickBookComponent, { className: "LandingQuickBookComponent", filePath: "apps/workplace/src/app/landing/landing-quick-book.component.ts", lineNumber: 82 });
})();
...
var LandingUpcomingComponent = class _LandingUpcomingComponent extends AsyncHandler {
  ...
  async remove(item, remove_series = false) {
    ...
    const remove_result = (item instanceof CalendarEvent && !item.from_bookings ? removeEvent : removeBooking)(item.id, {
...
//# debugId=781cc84e-d398-5687-a032-5738d62675fe
//# sourceMappingURL=landing.routes-FHUQXLED.js.map
