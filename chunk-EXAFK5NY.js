...
var BookingRulesFormComponent = class _BookingRulesFormComponent {
  constructor() {
    ...
    this.building_zones = toObservable(this._org.active_building).pipe(filter((_) => !!_), switchMap((bld) => uh({ parent_id: bld.id }).catch(() => ({ data: [] }))), map((res) => res.data), shareReplay(1));
  ...
  constructor() {
    ...
    this._booking_rules = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_booking_rules" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const { details } = await ic(params.building, `${this._data.type}_booking_rules`).catch(() => ({ details: [] }));
...
//# debugId=591e2096-3c49-54b7-a5b3-651add133ef4
//# sourceMappingURL=chunk-EXAFK5NY.js.map
