...
var AppSettingsModalComponent = class _AppSettingsModalComponent {
  ...
  async ngOnInit() {
    ...
    const zone_settings = (await rc(this.zone.id, `${this.workplace_key}`).catch(() => ({ details: {} }))).details;
    const parent_settings = (await rc(this.zone.parent_id, `${this.workplace_key}`).catch(() => ({ details: {} }))).details;
    const org_settings = (await rc(this._org.organisation.id, `${this.workplace_key}`).catch(() => ({ details: {} }))).details;
...
var AutoReleaseSettingsModalComponent = class _AutoReleaseSettingsModalComponent {
  ...
  async loadSettings(id) {
    ...
    const settings = (await pa({ parent_id: id })).data;
    const unencrypted = settings.find((_) => _.encryption_level === Le.None);
    if (!unencrypted)
      return;
    try {
  ...
  async save() {
    ...
    const settings = (await pa({ parent_id: this.id })).data;
    let unencrypted = settings.find((_) => _.encryption_level === Le.None);
    if (!unencrypted) {
      unencrypted = new Pe({
        parent_id: this.id,
    ...
    unencrypted.id ? await _a(unencrypted.id, unencrypted).catch(on_error) : await ma(unencrypted).catch(on_error);
    const metadata_key = this._settings.get("app.workplace_metadata_key") || "workplace_app";
    const metadata = await rc(this.id, metadata_key);
...
var BuildingFormComponent = class _BuildingFormComponent extends AsyncHandler {
  ...
  async saveChanges() {
    ...
    const building = await (data.id ? dh(data.id, body) : ph(body)).catch((e) => {
...
var InductionSettingsModalComponent = class _InductionSettingsModalComponent {
  ...
  async loadSettings() {
    ...
    const [bld_metadata, org_metadata, org_settings] = await Promise.all([
      rc(this._zone_id, visitor_kiosk_app),
      rc(this._org.organisation.id, visitor_kiosk_app),
      rc(this._org.organisation.id, "settings")
    ]);
  ...
  async save() {
    ...
    this._dialog_ref.disableClose = true;
    const metadata = await rc(this._zone_id, visitor_kiosk_app);
    const con_metadata = await rc(this._zone_id, concierge_app);
    ...
    const result_visitor = await oc(this._zone_id, {
    ...
    const result_concierge = await oc(this._zone_id, {
...
var ItemListModalComponent = class _ItemListModalComponent {
  ...
  async ngOnInit() {
    const metadata_key = this._settings.get("app.workplace_metadata_key") || "workplace_app";
    const metadata = await rc(this._bld_id, metadata_key);
  ...
  async save() {
    ...
    let resp = await oc(this._bld_id, {
      name: metadata_key,
    ...
    const concierge_metadata = await rc(this._bld_id, metadata_key);
    concierge_metadata.details.support_issue_types = items;
    resp = await oc(this._bld_id, {
      name: concierge_key,
...
var BookingPanelSettingsModalComponent = class _BookingPanelSettingsModalComponent extends AsyncHandler {
  ...
  async ngOnInit() {
    ...
    let settings = [];
    try {
      const response = await pa({ parent_id: this.zone.id });
      settings = response?.data || [];
    } catch {
    }
    const unencrypted_settings = settings.find((block) => block.encryption_level === Le.None);
  ...
  async save() {
    ...
    let unencrypted_settings = settings.find((block) => block.encryption_level === Le.None);
    if (!unencrypted_settings)
    ...
    const update = unencrypted_settings.id ? _a(unencrypted_settings.id, new_setting) : ma(new_setting);
    try {
      await update;
    } catch (e) {
      this._dialog_ref.disableClose = false;
...
var ConciergeSettingsFormModalComponent = class _ConciergeSettingsFormModalComponent {
  ...
  async _getMetadata(id) {
    const metadata = await rc(id, this.settings_key);
...
var LevelMapEditorModalComponent = class _LevelMapEditorModalComponent {
  ...
  async save() {
    ...
    try {
      ...
      const response = await dh(this._level.id, {
        id: this._level.id,
...
var LevelModalComponent = class _LevelModalComponent {
  ...
  async save() {
    ...
    const resp = await (data.id ? dh(data.id, __spreadProps(__spreadValues({}, data), {
      name: `LEVEL ${Mt().description} ${data.display_name}`
    })) : ph(__spreadProps(__spreadValues({}, data), {
      name: `LEVEL ${Mt().description} ${data.display_name}`
    }))).catch();
    if (resp.id)
...
//# debugId=ee88cad6-2dfa-5c53-86c2-301835824846
//# sourceMappingURL=zone-manager.routes-VK7GGAPP.js.map
