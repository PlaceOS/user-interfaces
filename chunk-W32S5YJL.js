...
async function updateEvent(id, data, q = {}, method = "patch") {
  const query = toQueryString(q);
  const item = await (method === "patch" ? se : le)(`${EVENTS_ENDPOINT}/${encodeURIComponent(id)}${query ? "?" + query : ""}`, new CalendarEvent(withAppVersion(data)).toJSON());
  return new CalendarEvent(item);
}
...
function removeEvent(id, q = {}) {
  const query = toQueryString(q);
  return J(`${EVENTS_ENDPOINT}/${encodeURIComponent(id)}${query ? "?" + query : ""}`, {
    response_type: "void"
  });
}
...
async function removeEventGuest(id, guest, q = {}) {
  const query = toQueryString(q);
  const item = await J(`${EVENTS_ENDPOINT}/${encodeURIComponent(id)}/attendee/${encodeURIComponent(guest.email)}${query ? "?" + query : ""}`);
  return new GuestUser(item);
}
...
function requestSpacesForZone(id) {
  ...
  SPACE_LIST_REQUESTS[id] = from(la({
    zone_id: id,
    limit: 500,
    signage: false
...
function generateEventForm(event = new CalendarEvent(), settings, injector) {
  ...
  const event_form = form(model, (p2) => {
    ...
    disabled(p2.assets, {
      when: ({ valueOf }) => !valueOf(p2.resources)?.length
    });
    disabled(p2.duration, {
      when: ({ valueOf }) => !!valueOf(p2.all_day)
    });
  }, { injector });
...
//# debugId=d86f842a-990a-5947-b276-b064f76a5822
//# sourceMappingURL=chunk-W32S5YJL.js.map
