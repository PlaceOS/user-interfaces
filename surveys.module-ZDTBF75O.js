...
var _SurveyService = class _SurveyService {
  ...
  async setSurvey(id) {
    ...
    const survey = await lastValueFrom(kl(id));
  ...
  async removeSurvey(survey, confirm = true) {
    if (confirm) {
      ...
      await lastValueFrom(Al(`${survey.id}`));
      result.close();
    } else {
      await lastValueFrom(Al(`${survey.id}`));
    }
  ...
  async removeQuestion(question, confirm = true) {
    if (confirm) {
      ...
      await lastValueFrom(vl(`${question.id}`));
      result.close();
    } else {
      await lastValueFrom(vl(`${question.id}`));
    }
  ...
  async _load(type = LoadType.ALL) {
    ...
    if (type === LoadType.ALL || type === LoadType.SURVEYS) {
      const surveys = await lastValueFrom(forkJoin(buildings.map((bld) => Sl({ building_id: bld.id }))));
      const survey_list = surveys.flat();
      this.survey_list.set(survey_list);
    }
    if (type === LoadType.ALL || type === LoadType.ANSWERS) {
      const answers = await lastValueFrom(forkJoin(this.survey_list().map((survey) => al({ survey_id: survey.id }))));
      this.answer_list.set(answers.flat());
    }
    if (type === LoadType.ALL || type === LoadType.QUESTIONS) {
      const questions = await lastValueFrom(ml({ limit: 1e3 }));
...
var _QuestionModalComponent = class _QuestionModalComponent {
  ...
  async save() {
    ...
    if (!question_el.valid)
      return;
    this.loading = true;
    const call = this.is_edit ? bl(`${this.question.id}`, question_el.question()) : gl(question_el.question());
...
var _NewSurveyService = class _NewSurveyService {
  constructor() {
    ...
    this.survey_list$ = combineLatest([
      ...
      this._change
    ]).pipe(tap(() => this._loading.next(true)), switchMap(([list]) => forkJoin(list.map((bld) => Sl({ building_id: bld.id })))), map((data) => flatten(data)), tap((list) => this.survey_list.set(list)), shareReplay(1));
    this.answer_list$ = this.survey_list$.pipe(switchMap((surveys) => forkJoin(surveys.map((survey) => al({ survey_id: survey.id })))), map((answers) => flatten(answers)), tap(() => this._loading.next(false)), tap((list) => this.answer_list.set(list)), shareReplay(1));
    ...
    this.survey$ = this._survey_id.pipe(switchMap((id) => !id ? of(null) : kl(id)), shareReplay(1));
    this.questions$ = ml({
      limit: 1e3
    }).pipe(
...
var _QuestionPipe = class _QuestionPipe extends AsyncHandler {
  transform(id) {
    if (!QUESTION_MAP[id]) {
      lastValueFrom(yl(id)).then((question) => QUESTION_MAP[id] = question);
    }
...
var _SurveyBuilderComponent = class _SurveyBuilderComponent extends AsyncHandler {
  ...
  async saveSurvey() {
    ...
    const call = this.form.value.id ? xl(survey) : $l(`${survey.id}`, survey);
...
var _SurveyResponsesComponent = class _SurveyResponsesComponent extends AsyncHandler {
  constructor() {
    ...
    this.answers$ = combineLatest([
      ...
      return al(q).pipe(catchError(() => of([])));
...
//# sourceMappingURL=surveys.module-ZDTBF75O.js.map
