...
| REG-10 | P1 | The booking form does not discard user input while it is still initialising. | **[PPT-2643](https://acaprojects.atlassian.net/browse/PPT-2643)** | **blocked** — the form is rebuilt when async init completes and restores defaults (title, All Day, Require locker), silently dropping anything typed before that. A real user can hit this; they would just see their title or options revert. `bookDeskViaUI` converges on the state to work around it, which means **the suite no longer detects it** — hence this row. Investigated 2026-08-05 against #478 (`a0360486`); **no evidence the fix is incomplete**, and the shipped suite is stable — 6 consecutive full runs, 8/8, at `--retries=0`. Removing the block could not be shown to be safe *or* unsafe here: the race needs a slow machine, and this one wins it every time. Two failed attempts to prove otherwise, both recorded so nobody repeats them: (1) a synthetic "type during init, assert it survives" spec passed with *and* without the fix, even with the API responses held to widen the window; (2) removing the block appeared to prove the bug survived — it did not. That red was a Playwright **strict-mode violation**, not a reverted value: opening the desk-select modal puts a second "All Day" checkbox in the DOM (`desk-filters`, bound to the same field), so an unscoped locator matched two elements and threw, with both checked. Scoping the locator to `desk-flow-form` then broke it a second way, because `setCheckbox` silently returns when its locator matches nothing, turning a narrower scope into a no-op and a genuinely invalid form. Both experiments were reverted. **The block stays, this row stays blocked, and settling it needs artificial slowness (throttled CPU) or a unit test driving the deferred branch — not another e2e attempt on fast hardware.** |
...
- **REG-10 is invisible to this suite by design.** `bookDeskViaUI` converges on the form state,
  ...
  *complete* is genuinely unresolved, and the honest summary is that this suite cannot answer it
  ...
  Nothing was changed in the flow helper in the end, and that is the finding: two attempts to
  replace the workaround with a real assertion both produced red runs that looked like the app
  bug and were not. The lesson is procedural — when a test goes red, confirm the *shipped* code
  is green in the same environment before concluding anything about the app. Running the
  unmodified helper three times (8/8 each) is what separated "the fix is incomplete" from "my
  edit is wrong", and only the second was true.
