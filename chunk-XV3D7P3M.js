...
} from "./chunk-ZF6RDJ6W.js";
import {
  ExploreStateService
} from "./chunk-BAQQMQIN.js";
import {
...
var WFHSettingsModalComponent = class _WFHSettingsModalComponent {
  ...
  async saveChanges(close = true) {
    ...
    if (!this._data?.local) {
      const user = await ja("current");
      await Ba(user.id, __spreadProps(__spreadValues({}, user), {
...
var ExploreSearchService = class _ExploreSearchService {
  ...
  constructor() {
    ...
    this._space_search = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_space_search" } : (
      ...
      params: () => ({ q: this._debounced_filter.value() }),
      loader: ({ params: { q } }) => q?.length > 2 ? la({ q, zone_id: this._org.organisation.id }).then(({ data }) => data.filter((_) => _.map_id).map((_) => new Space(__spreadProps(__spreadValues({}, _), {
    ...
    this._desk_search = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_desk_search" } : (
      ...
      loader: ({ params: bld }) => {
        ...
        return sc(bld.id, { name: "desks" }).then((i) => flatten(i.map((j) => (j.metadata.desks?.details || []).map((k) => new Desk(__spreadProps(__spreadValues({}, k), { zone: j.zone })))))).catch(() => []);
      }
    }));
    ...
    this._map_features = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_map_features" } : (
      ...
      params: () => this._building() || void 0,
      loader: async () => {
        const data = await sc(this._org.building.id, {
          name: "map_features"
        }).catch(() => []);
        const list2 = [];
    ...
    this.search_fn = (q) => this._settings.get("app.basic_user_search") ? La({ q, authority_id: It()?.id }).then((_) => _.data) : searchStaff(q);
    this.init();
  }
  async init() {
  ...
  _loadSpaceBookings() {
    effect(async (onCleanup) => {
      ...
      for (const [idx, space] of (spaces || []).entries()) {
        const binding = gd(space.id, "Bookings").variable("bookings");
...
//# debugId=683bc63e-3abf-5a7c-ba46-26eb7dc8dea5
//# sourceMappingURL=chunk-XV3D7P3M.js.map
