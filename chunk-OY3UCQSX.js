...
var TeamScheduleService = class _TeamScheduleService {
  ...
  async _fetchColleagues() {
    try {
      ...
      if (!user?.id)
        return [];
      const metadata = await ju(user.id, "contacts").catch(() => ({}));
      const list = metadata?.details instanceof Array ? metadata.details : [];
      const users = await Promise.all(list.map((contact) => Ca(contact.email).catch(() => contact)));
...
//# sourceMappingURL=chunk-OY3UCQSX.js.map
