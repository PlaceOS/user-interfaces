...
async function queryAllAssetPages(query = {}) {
  let response = await jh(__spreadProps(__spreadValues({}, query), {
...
function saveAsset(asset) {
  return asset.id ? Bh(asset.id, asset) : Wh(asset);
}
function saveAssetPurchaseOrder(order) {
  return order.id ? al(order.id, order) : hl(order);
}
...
//# sourceMappingURL=chunk-6YA3SFB5.js.map
