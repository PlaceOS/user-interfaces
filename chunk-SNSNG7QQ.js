...
var _ExploreDeviceInfoComponent = class _ExploreDeviceInfoComponent extends AsyncHandler {
  ...
  async loadUser() {
    if (this.username)
      return;
    const mod = Qa(this._details.system, "LocationServices");
    if (!mod)
      return;
...
var _ExploreDesksService = class _ExploreDesksService extends AsyncHandler {
  constructor() {
    ...
    this.booking_rules = this._org.active_building.pipe(filter((bld) => !!bld), switchMap((bld) => Pu(bld.id, `desk_booking_rules`).pipe(catchError(() => of({ details: [] })))), map((_) => _?.details instanceof Array ? _.details : []), shareReplay(1));
    this.desk_list = this._state.level.pipe(debounceTime(50), switchMap((lvl) => Pu(lvl.id, "desks").pipe(catchError(() => of({ details: [] })), map((i) => (i?.details instanceof Array ? i.details : []).map((j) => new Desk(__spreadProps(__spreadValues({}, j), { zone: lvl })))))), catchError((e) => []), shareReplay(1));
...
var _ExploreParkingService = class _ExploreParkingService extends AsyncHandler {
  constructor() {
    ...
    this.booking_rules = this._org.active_building.pipe(filter((bld) => !!bld), switchMap((bld) => Pu(bld.id, `parking_booking_rules`).pipe(catchError(() => of({ details: [] })))), map((_) => _?.details instanceof Array ? _.details : []), shareReplay(1));
    ...
    this.spaces = this.levels.pipe(switchMap((_) => forkJoin(_.map((l) => Pu(l.id, "parking-spaces").pipe(map((d) => (d.details instanceof Array ? d.details : []).map((s) => __spreadProps(__spreadValues({}, s), { zone_id: l.id }))))))), map((_) => flatten(_)), shareReplay(1));
...
var _ExplorePointOfInterestService = class _ExplorePointOfInterestService extends AsyncHandler {
  constructor() {
    ...
    this._explore = inject(ExploreStateService);
    this._dialog = inject(MatDialog);
    this._features = this._org.active_building.pipe(switchMap(() => Pu(this._org.organisation.id, "points-of-interest").pipe(catchError((_) => of({ details: {} })))), map((_) => {
...
}, features: [\u0275\u0275InheritDefinitionFeature], attrs: _c1, decls: 2, vars: 4, consts: [["canvas", ""], [1, "absolute", "left-1/2", "top-1/2", "-translate-x-1/2", "-translate-y-1/2"]], template: function MapCanvasComponent_Template(rf, ctx) {
  if (rf & 1) {
...
var _ExploreZonesService = class _ExploreZonesService extends AsyncHandler {
  ...
  async init() {
    await firstTruthyValueFrom(this._org.initialised);
    const zone_metadata = await Promise.all(this._org.levels.map((bld) => Pu(bld.id, "map_regions").toPromise()));
    this._area_list = [];
...
var _ExploreSearchService = class _ExploreSearchService {
  ...
  constructor() {
    ...
    this._role_assigned_contacts = this._org.active_building.pipe(filter((bld) => !!bld), switchMap((bld) => Pu(bld.id, "emergency_contacts")), map(({ details }) => details?.contacts || []), shareReplay(1));
    ...
    this._space_search = this._filter.pipe(debounceTime(400), tap(() => this._loading.next(true)), switchMap((q) => q?.length > 2 ? $c({ q, zone_id: this._org.organisation.id }).pipe(map(({ data }) => data.filter((_) => _.map_id).map((_) => new Space(__spreadProps(__spreadValues({}, _), {
      level: this._org.levelWithID(_.zones)
    }))))) : of([])), catchError(() => []));
    this._desk_search = combineLatest([
      this._org.active_building
    ]).pipe(debounceTime(400), tap(() => this._loading.next(true)), switchMap(([bld]) => bld ? Du(bld.id, { name: "desks" }).pipe(catchError(() => of([])), map((i) => flatten(i.map((j) => (j.metadata.desks?.details || []).map((k) => new Desk(__spreadProps(__spreadValues({}, k), { zone: j.zone }))))))) : of([])), catchError(() => []));
    ...
    this._map_features = this._org.active_building.pipe(filter((bld) => !!bld), switchMap(() => Du(this._org.building.id, {
      name: "map_features"
    ...
    this._poi_metadata = this._org.initialised.pipe(filter((_) => _), switchMap(() => Pu(this._org.organisation.id, "points-of-interest").pipe(catchError((_) => of({ details: {} })))), shareReplay(1));
    this._poi_list = combineLatest([
      this._org.active_building,
      this._poi_metadata
    ...
    this.search_fn = (q) => this._settings.get("app.basic_user_search") ? Jc({ q, authority_id: kt()?.id }).pipe(map((_) => _.data)) : searchStaff(q);
...
//# sourceMappingURL=chunk-SNSNG7QQ.js.map
