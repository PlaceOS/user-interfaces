...
var ZonePipe = class _ZonePipe {
  async transform(id) {
    if (ZONE_LIST[id])
      return ZONE_LIST[id];
    const zone = await sh(id).catch((_) => null);
    if (!zone)
      return EMPTY_ZONE;
    ZONE_LIST[id] = zone;
    return zone;
...
var SignageApprovePlaylistModalComponent = class _SignageApprovePlaylistModalComponent {
  ...
  async undoChanges() {
    ...
    await Mh(this._playlist?.id, previous.items);
    this.loading.set("");
    this._dialog_ref.disableClose = false;
...
var SignageDisplayModalComponent = class _SignageDisplayModalComponent {
  ...
  async save() {
    ...
    const method = this.display.id ? fa(this.display.id, new_display) : _a(new_display);
...
var SignageMediaModalComponent = class _SignageMediaModalComponent {
  ...
  async saveMedia() {
    ...
    if (this.item.id) {
      await yh(this.item.id, new_media).catch(onError);
    } else {
...
var SignageStateService = class _SignageStateService extends AsyncHandler {
  constructor() {
    ...
    this._media = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_media" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const list = await queryAllPages(dh({ limit: 200 }));
    ...
    this._playlists = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_playlists" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const list = await queryAllPages(Sh({ limit: 200 }));
    ...
    this._displays = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_displays" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const list = await queryAllPages(la({
    ...
    this._zones = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_zones" } : (
      ...
      defaultValue: [],
      loader: async () => {
        const list = await queryAllPages(th({
          limit: 200,
          tags: "signage"
        })).catch(() => []);
  ...
  async savePlaylist(playlist) {
    ...
    const call = clean_playlist.id ? Ah(clean_playlist.id, clean_playlist, "put") : qh(clean_playlist);
  ...
  previewFileMedia(media, playlist_id = "") {
    ...
    ref.componentInstance.save.subscribe(async () => {
      ...
      if (playlist_id && new_media.id) {
        const media_list = await Th(playlist_id);
        const new_media_list = [...media_list.items, new_media.id];
        await Mh(playlist_id, new_media_list);
      }
      ref.close();
    });
  }
    ...
    this.playlist_thumbnail_media = computed(
      () => {
        ...
        for (const [playlist_id, data] of Object.entries(counts)) {
          result[playlist_id] = (data.media_ids || []).map((id) => vh(id));
        }
        return result;
      },
  ...
  constructor() {
    ...
    this._zones = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_zones" } : (
      ...
      loader: async ({ params: term }) => {
        const resp = await th(__spreadProps(__spreadValues({}, this.query), {
          q: term,
          limit: 100
        })).catch(() => ({ data: [] }));
        return resp.data;
...
var SignageDisplaysComponent = class _SignageDisplaysComponent extends AsyncHandler {
  ...
  async setOrientation(orientation) {
    ...
    if (!display)
      return;
    await fa(display.id, { orientation, version: display.version }, "patch").catch((e) => {
...
var SignageMediaListComponent = class _SignageMediaListComponent {
  ...
  async addToPlaylist(media_id, playlist) {
    const media_list = await Th(playlist.id);
  ...
  constructor() {
    ...
    this._playlist_media_resource = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_playlist_media_resource" } : (
      ...
      loader: async ({ params }) => {
        ...
        this.loading.set(true);
        try {
          const result = await Th(params.id).catch(() => ({ id: "", items: [], approved: 0 }));
...
//# debugId=e6048712-1bf6-5000-9cc1-7fa855959ac4
//# sourceMappingURL=signage.routes-UEULQ4WV.js.map
