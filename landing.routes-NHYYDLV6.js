...
import {
  FAV_PARKING_KEY
} from "./chunk-VSMID3DS.js";
import "./chunk-F2UPKP2N.js";
...
import "./chunk-Q6RT7OBK.js";
import "./chunk-TXXS7VWJ.js";
...
var LandingStateService = class _LandingStateService extends AsyncHandler {
  constructor() {
    ...
    this.search_fn = async (q) => this._settings.get("app.basic_user_search") || this._settings.get("app.colleagues_require_auth") !== false ? Ja({ q, authority_id: Mt()?.id }).then(({ data }) => data.map((_) => new StaffUser(_))) : searchStaff(q);
    ...
    effect((onCleanup) => {
      ...
      for (const [idx, space] of list.entries()) {
        const binding = zd(space.id, "Bookings").variable("status");
  ...
  async updateContacts() {
    const metadata = await rc(currentUser().id, "contacts");
    const list = metadata.details instanceof Array ? metadata.details : [];
    const users = await Promise.all(list.map((_) => Va(_.email).catch(() => _)));
...
var LandingFavouritesComponent = class _LandingFavouritesComponent extends AsyncHandler {
  ...
  async ngOnInit() {
    const metadata = await rc(this._org.organisation.id, "room_alerts");
...
//# sourceMappingURL=landing.routes-NHYYDLV6.js.map
