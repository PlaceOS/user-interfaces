...
var ZonePipe = class _ZonePipe {
  async transform(id) {
    if (ZONE_LIST[id])
      return ZONE_LIST[id];
    const zone = await Ja(id).catch((_) => null);
    if (!zone)
      return EMPTY_ZONE;
    ZONE_LIST[id] = zone;
    return zone;
...
var SignageApprovePlaylistModalComponent = class _SignageApprovePlaylistModalComponent {
  ...
  async undoChanges() {
    ...
    await Sh(this._playlist?.id, previous.items);
    this.loading.set("");
    this._dialog_ref.disableClose = false;
...
var SignageDisplayModalComponent = class _SignageDisplayModalComponent {
  ...
  async save() {
    ...
    const method = this.display.id ? ua(this.display.id, new_display) : ca(new_display);
...
var SignageMediaModalComponent = class _SignageMediaModalComponent {
  ...
  async saveMedia() {
    ...
    if (this.item.id) {
      await ch(this.item.id, new_media).catch(onError);
    } else {
...
var SignagePlaylistModalComponent = class _SignagePlaylistModalComponent {
  constructor() {
    this._data = inject(MAT_DIALOG_DATA) ?? new or({});
...
var SignageStateService = class _SignageStateService extends AsyncHandler {
  constructor() {
    ...
    this._media = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_media" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const resp = await rh({ limit: 2500 });
  ...
  async editPlaylist(playlist = new or({})) {
  ...
  async editMedia(media = new Rs({}), file, playlist_id = "") {
  ...
  async savePlaylist(playlist) {
    ...
    const call = clean_playlist.id ? _h(clean_playlist.id, clean_playlist, "put") : mh(clean_playlist);
  ...
  async getPlaylistMedia(playlist_id) {
    const { items } = await yh(playlist_id);
    return items;
  }
  previewMedia(item) {
  ...
  previewFileMedia(media, playlist_id = "") {
    ...
    ref.componentInstance.save.subscribe(async () => {
      ...
      if (playlist_id && new_media.id) {
        ...
        await Sh(playlist_id, new_media_list);
      }
      ref.close();
    });
  }
  async addMediaFromLink(url, media_item = new Rs({})) {
  ...
  constructor() {
    ...
    this._zones = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_zones" } : (
      ...
      loader: async ({ params: term }) => {
        const resp = await Ka(__spreadProps(__spreadValues({}, this.query), {
          q: term,
          limit: 100
        })).catch(() => ({ data: [] }));
        return resp.data;
...
var SignageDisplaysComponent = class _SignageDisplaysComponent extends AsyncHandler {
  ...
  async setOrientation(orientation) {
    ...
    if (!display)
      return;
    await ua(display.id, { orientation, version: display.version }, "patch").catch((e) => {
  ...
  constructor() {
    ...
    this._playlist_media_resource = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_playlist_media_resource" } : (
      ...
      loader: async ({ params }) => {
        ...
        this.loading.set(true);
        try {
          const result = await yh(params.id).catch(() => ({ id: "", items: [], approved: 0 }));
...
//# sourceMappingURL=signage.routes-QWUTL5JV.js.map
