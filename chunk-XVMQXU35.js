...
var SignageService = class _SignageService {
  ...
  async addPlaylist() {
    if (!this._requirePermission(this.can_create(), i18n("SIGNAGE_MANAGER.SVC_NO_CREATE_PLAYLISTS")))
      return;
    const { PlaylistEditModalComponent } = await import("./playlist-edit-modal.component-HJEIX3HW.js");
  ...
  async approvePlaylist(playlist) {
    ...
    if (!this._requirePermission(this.can_approve(), i18n("SIGNAGE_MANAGER.SVC_NO_APPROVE_PLAYLISTS")))
      return;
    const { PlaylistApproveModalComponent } = await import("./playlist-approve-modal.component-7U3DAUUA.js");
  ...
  async requestPlaylistApproval(playlist) {
    ...
    if (!group)
      return;
    const { PlaylistRequestApprovalModalComponent } = await import("./playlist-request-approval-modal.component-36WNWSCW.js");
  ...
  async editPlaylistItemSchedules(items) {
    ...
    if (!this._requirePermission(this.can_update(), i18n("SIGNAGE_MANAGER.SVC_NO_UPDATE_PLAYLISTS")))
      return false;
    const { PlaylistItemScheduleModalComponent } = await import("./playlist-item-schedule-modal.component-GMSRPUYZ.js");
  ...
  async addTemplate() {
    if (!this._requirePermission(this.can_create(), i18n("SIGNAGE_MANAGER.SVC_NO_CREATE_TEMPLATES")))
      return;
    const { TemplateEditModalComponent } = await import("./template-edit-modal.component-HUKCZP4D.js");
  ...
  async editTemplateMapping(target, mapping = null) {
    ...
    const { TemplateMappingModalComponent } = await import("./template-mapping-modal.component-SYKITCPC.js");
  ...
  async approveTemplate(template) {
    ...
    if (!this._requirePermission(this.can_approve(), i18n("SIGNAGE_MANAGER.SVC_NO_APPROVE_TEMPLATES")))
      return;
    const { TemplateApproveModalComponent } = await import("./template-approve-modal.component-SV64X4FJ.js");
  ...
  async requestTemplateApproval(template) {
    ...
    if (!group)
      return;
    const { TemplateRequestApprovalModalComponent } = await import("./template-request-approval-modal.component-HUM7OJEP.js");
  ...
  async previewMedia(item) {
    ...
    const { MediaPreviewModalComponent } = await import("./media-preview-modal.component-ISHECFIT.js");
  ...
  async bulkUploadMedia(files, playlist_id = "") {
    ...
    const { BulkMediaUploadModalComponent } = await import("./bulk-media-upload-modal.component-B6UVIPP6.js");
  ...
  async generateMediaWithAI(options = {}) {
    ...
    if (this._ai_modal_ref)
      return;
    const { AiImageModalComponent } = await import("./ai-image-modal.component-HP465VMN.js");
  ...
  async editMedia(media = new ws({}), file, playlist_id = "", prepared_file_metadata, plugin) {
    ...
    const { MediaEditModalComponent } = await import("./media-edit-modal.component-7JU3MRS6.js");
  ...
  async addMediaTags(items) {
    ...
    if (!this._requirePermission(this.can_update(), i18n("SIGNAGE_MANAGER.SVC_NO_UPDATE_MEDIA")))
      return false;
    const { MediaTagsModalComponent } = await import("./media-tags-modal.component-L47KI2QB.js");
  ...
  async openPlaylistSelectModal(media_id) {
    const { PlaylistSelectModalComponent } = await import("./playlist-select-modal.component-LYPJSTIX.js");
  ...
  async addDisplayToZone(zone) {
    if (!this._requirePermission(this.can_update(), i18n("SIGNAGE_MANAGER.SVC_NO_UPDATE_ASSIGNMENTS")))
      return;
    const { DisplaySelectModalComponent } = await import("./display-select-modal.component-TMXVT2CY.js");
  ...
  async addZoneToPlaylist(playlist) {
    if (!this._requirePermission(this.can_update(), i18n("SIGNAGE_MANAGER.SVC_NO_UPDATE_ASSIGNMENTS")))
      return;
    const { ZoneSelectModalComponent } = await import("./zone-select-modal.component-2GUECCG3.js");
...
//# sourceMappingURL=chunk-XVMQXU35.js.map
