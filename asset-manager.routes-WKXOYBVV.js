...
var AssetManagerStateService = class _AssetManagerStateService extends AsyncHandler {
  constructor() {
    ...
    this._settings_data = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_settings_data" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return {};
        const metadata = await rc(params.building, "assets-settings").catch(() => ({}));
  ...
  async getConfig(zone_id = this._org.building.id) {
    const rules = (await rc(zone_id, "assets_config")).details;
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        ...
        const asset = await Gl(params.get("id")).catch(() => null);
        if (!asset) {
      ...
      if (params.get("group_id")) {
        ...
        const product = await Yl(params.get("group_id")).catch(() => null);
        if (!product) {
...
var AssetGroupFormComponent = class _AssetGroupFormComponent extends AsyncHandler {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        this.loading.set("Loading Product Details...");
        const product = await Yl(params.get("id")).catch(() => null);
        if (!product) {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        ...
        const asset = await cd(params.get("id")).catch(() => null);
        if (!asset) {
...
//# debugId=7db104b5-0006-5315-b8ba-7fe3b3208fcd
//# sourceMappingURL=asset-manager.routes-WKXOYBVV.js.map
