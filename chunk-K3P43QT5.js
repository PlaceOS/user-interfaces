...
var SignageService = class _SignageService {
  ...
  async addPlaylist() {
    if (!this._requirePermission(this.can_create(), i18n("SIGNAGE_MANAGER.SVC_NO_CREATE_PLAYLISTS")))
      return;
    const { PlaylistEditModalComponent } = await import("./playlist-edit-modal.component-5HUDYN6V.js");
  ...
  async approvePlaylist(playlist) {
    ...
    if (!this._requirePermission(this.can_approve(), i18n("SIGNAGE_MANAGER.SVC_NO_APPROVE_PLAYLISTS")))
      return;
    const { PlaylistApproveModalComponent } = await import("./playlist-approve-modal.component-6GXZDXNR.js");
  ...
  async requestPlaylistApproval(playlist) {
    ...
    if (!group)
      return;
    const { PlaylistRequestApprovalModalComponent } = await import("./playlist-request-approval-modal.component-73N24MMD.js");
  ...
  async editPlaylistItemSchedules(items) {
    ...
    if (!this._requirePermission(this.can_update(), i18n("SIGNAGE_MANAGER.SVC_NO_UPDATE_PLAYLISTS")))
      return false;
    const { PlaylistItemScheduleModalComponent } = await import("./playlist-item-schedule-modal.component-W6OPJKD4.js");
  ...
  async addTemplate() {
    if (!this._requirePermission(this.can_create(), i18n("SIGNAGE_MANAGER.SVC_NO_CREATE_TEMPLATES")))
      return;
    const { TemplateEditModalComponent } = await import("./template-edit-modal.component-6ZQN3SMM.js");
  ...
  async editTemplateMapping(target, mapping = null) {
    ...
    const { TemplateMappingModalComponent } = await import("./template-mapping-modal.component-QU755SQX.js");
  ...
  async approveTemplate(template) {
    ...
    if (!this._requirePermission(this.can_approve(), i18n("SIGNAGE_MANAGER.SVC_NO_APPROVE_TEMPLATES")))
      return;
    const { TemplateApproveModalComponent } = await import("./template-approve-modal.component-J3UE4YXD.js");
  ...
  async requestTemplateApproval(template) {
    ...
    if (!group)
      return;
    const { TemplateRequestApprovalModalComponent } = await import("./template-request-approval-modal.component-YFQJX2ML.js");
  ...
  async previewMedia(item) {
    ...
    const { MediaPreviewModalComponent } = await import("./media-preview-modal.component-YOSIGXJC.js");
  ...
  async bulkUploadMedia(files, playlist_id = "") {
    ...
    const { BulkMediaUploadModalComponent } = await import("./bulk-media-upload-modal.component-EMS26MXF.js");
  ...
  async generateMediaWithAI(options = {}) {
    ...
    if (this._ai_modal_ref)
      return;
    const { AiImageModalComponent } = await import("./ai-image-modal.component-H4PCIDER.js");
  ...
  async editMedia(media = new ws({}), file, playlist_id = "", prepared_file_metadata, plugin) {
    ...
    const { MediaEditModalComponent } = await import("./media-edit-modal.component-NTVJERIE.js");
  ...
  async addMediaTags(items) {
    ...
    if (!this._requirePermission(this.can_update(), i18n("SIGNAGE_MANAGER.SVC_NO_UPDATE_MEDIA")))
      return false;
    const { MediaTagsModalComponent } = await import("./media-tags-modal.component-3RPWLCEE.js");
  ...
  async openPlaylistSelectModal(media_id) {
    const { PlaylistSelectModalComponent } = await import("./playlist-select-modal.component-QRE22IH4.js");
  ...
  async addDisplayToZone(zone) {
    if (!this._requirePermission(this.can_update(), i18n("SIGNAGE_MANAGER.SVC_NO_UPDATE_ASSIGNMENTS")))
      return;
    const { DisplaySelectModalComponent } = await import("./display-select-modal.component-C2OI7CN3.js");
  ...
  async addZoneToPlaylist(playlist) {
    if (!this._requirePermission(this.can_update(), i18n("SIGNAGE_MANAGER.SVC_NO_UPDATE_ASSIGNMENTS")))
      return;
    const { ZoneSelectModalComponent } = await import("./zone-select-modal.component-7FCSPNXN.js");
...
//# sourceMappingURL=chunk-K3P43QT5.js.map
