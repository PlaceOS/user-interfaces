...
var AssetManagerStateService = class _AssetManagerStateService extends AsyncHandler {
  constructor() {
    ...
    this._settings_data = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_settings_data" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return {};
        const metadata = await Wu(params.building, "assets-settings").catch(() => ({}));
  ...
  async postForm() {
    ...
    data.other_data = Vs(other_data, [void 0, null, ""]);
  ...
  async getConfig(zone_id = this._org.building.id) {
    const rules = (await Wu(zone_id, "assets_config")).details;
...
var AssetBulkFormComponent = class _AssetBulkFormComponent extends AsyncHandler {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        ...
        const asset = await Jh(params.get("id")).catch(() => null);
        if (!asset) {
      ...
      if (params.get("group_id")) {
        ...
        const product = await il(params.get("group_id")).catch(() => null);
        if (!product) {
  ...
  async save() {
    ...
    const data = this.model();
    const list = await el(new Array(this.count()).fill(__spreadProps(__spreadValues({}, data), {
      zone_id: this._org.building.id
    }))).catch((e) => {
      this.loading.set("");
...
var AssetGroupFormComponent = class _AssetGroupFormComponent extends AsyncHandler {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        this.loading.set("Loading Product Details...");
        const product = await il(params.get("id")).catch(() => null);
        if (!product) {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        ...
        const asset = await pl(params.get("id")).catch(() => null);
        if (!asset) {
...
//# sourceMappingURL=asset-manager.routes-6R4SFFLF.js.map
