...
var _SignageApprovePlaylistModalComponent = class _SignageApprovePlaylistModalComponent {
  constructor() {
    ...
    this.loading = signal("");
    this.playlist_versions = this._playlist_id.pipe(filter((_) => !!_), tap(() => this.loading.set("Loading versions...")), switchMap((id) => ka(id, { limit: 2 })), shareReplay(1));
  ...
  async undoChanges() {
    ...
    await lastValueFrom($a(this._playlist.id, previous.items));
    this.loading.set("");
  ...
  async approve() {
    ...
    this._dialog_ref.disableClose = true;
    await lastValueFrom(xa(this._playlist.id));
    this.loading.set("");
    this._dialog_ref.disableClose = false;
...
var _SignageDisplayModalComponent = class _SignageDisplayModalComponent {
  ...
  async save() {
    ...
    const new_display = new Tr(__spreadProps(__spreadValues({}, form_value), {
    ...
    const method = this.display.id ? Oc(this.display.id, new_display) : Ec(new_display);
...
var _SignageMediaModalComponent = class _SignageMediaModalComponent {
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      animation: new FormControl(Dr.Default),
  ...
  async saveMedia() {
    ...
    if (this.item.id) {
      await lastValueFrom(da(this.item.id, new_media)).catch(onError);
    } else {
...
var _SignagePlaylistModalComponent = class _SignagePlaylistModalComponent {
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      default_animation: new FormControl(Dr.Cut),
...
var _SignageStateService = class _SignageStateService extends AsyncHandler {
  constructor() {
    ...
    this.media = combineLatest([
      ...
      this._change
    ]).pipe(filter(([_]) => !!_?.id), debounceTime(300), switchMap(() => fa({ limit: 2500 })), map((_) => _.data.sort((a, b) => b.created_at - a.created_at)), shareReplay(1));
    this.playlists = combineLatest([
      this._org.active_building,
      this._change
    ]).pipe(filter(([_]) => !!_?.id), debounceTime(300), switchMap(() => ma({ limit: 500 })), map((_) => (_.data || []).sort((a, b) => a.name.localeCompare(b.name))), shareReplay(1));
    this.displays = combineLatest([
      this._org.active_region,
      this._org.active_building,
      this._change
    ]).pipe(filter(([, bld]) => !!bld?.id), switchMap(([region, bld]) => $c({
  ...
  async editDisplay(display = new Tr({})) {
  ...
  async removeDisplay(display) {
    ...
    if (display.map_id || display.email || display.module_list.length > 0) {
      await lastValueFrom(Oc(display.id, {
        signage: false
      }));
    } else {
      await lastValueFrom(Ic(display.id));
    }
    this._change.next(Date.now());
  ...
  async savePlaylist(playlist) {
    const call = lastValueFrom(playlist.id ? ga(playlist.id, playlist, "put") : ba(playlist));
  ...
  previewFileMedia(media, playlist_id = "") {
    ...
    ref.componentInstance.save.subscribe(async () => {
      ...
      if (playlist_id && new_media.id) {
        const media_list = await lastValueFrom(Sa(playlist_id));
        const new_media_list = [...media_list.items, new_media.id];
        await lastValueFrom($a(playlist_id, new_media_list));
      }
      ref.close();
    });
  }
  async addMediaFromLink(url, media_item = new Ko({})) {
    ...
    const result = await lastValueFrom(pa(data));
  ...
  async updateMedia(item) {
    if (!item?.id)
      return;
    await lastValueFrom(da(item.id, item));
  }
...
var _ZoneSelectModalComponent = class _ZoneSelectModalComponent {
  constructor() {
    ...
    this.zones = this.search_term.pipe(debounceTime(300), switchMap((term) => ta(__spreadProps(__spreadValues({}, this.query), { q: term, limit: 100 })).pipe(map((_) => _.data), catchError(() => of([])))), map((zones) => zones.filter((zone) => !this.ignore.includes(zone.id))), startWith([]), shareReplay(1));
  }
};
...
var _SignageDisplaysComponent = class _SignageDisplaysComponent {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      jc(id).pipe(map((_) => _.data), tap((_) => setTimeout(() => this.switching.set(false), 200))).subscribe((data) => this._triggers.set(data));
    });
  }
  ...
  async setOrientation(orientation) {
    ...
    if (!display)
      return;
    await lastValueFrom(Oc(display.id, { orientation, version: display.version }, "patch")).catch((e) => {
...
var _SignageMediaListComponent = class _SignageMediaListComponent {
  ...
  async addToPlaylist(media_id, playlist) {
    const media_list = await lastValueFrom(Sa(playlist.id));
...
var _SignagePlaylistMediaListComponent = class _SignagePlaylistMediaListComponent {
  constructor() {
    ...
    this._playlist_media = toSignal(toObservable(this._playlist_media_observable).pipe(filter((playlist) => !!playlist), debounceTime(300), switchMap((playlist) => Sa(playlist.id).pipe(catchError(() => of({ id: "", items: [], approved: 0 })))), tap((_) => this.approved.set(_.approved))), { initialValue: { id: "", items: [], approved: 0 } });
    this.media = computed(() => {
      const playlist = this._playlist_media();
...
var _SignageMediaComponent = class _SignageMediaComponent extends AsyncHandler {
  ...
  async drop(playlist, event) {
    ...
    const media_list = await Sa(playlist.id).toPromise();
...
var _SignageZonesComponent = class _SignageZonesComponent {
  constructor() {
    ...
    effect(() => {
      ...
      ua(id).pipe(map((_) => _.data), tap((_) => setTimeout(() => this.switching.set(false), 100))).subscribe((data) => this._triggers.set(data));
    });
  }
  async addPlaylist(playlist) {
    const zone = this.active_zone();
...
//# sourceMappingURL=signage.module-UFLTJXVG.js.map
