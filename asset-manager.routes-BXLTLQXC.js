...
var AssetBulkFormComponent = class _AssetBulkFormComponent extends AsyncHandler {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      ...
      if (params.get("group_id")) {
        ...
        const product = await Al(params.get("group_id")).catch(() => null);
        if (!product) {
  ...
  async save() {
    ...
    const data = this.model();
    const list = await vl(new Array(this.count()).fill(__spreadProps(__spreadValues({}, data), {
      zone_id: this._org.building.id
    }))).catch((e) => {
      this.loading.set("");
...
var AssetGroupFormComponent = class _AssetGroupFormComponent extends AsyncHandler {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        this.loading.set("Loading Product Details...");
        const product = await Al(params.get("id")).catch(() => null);
        if (!product) {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        ...
        const asset = await Cl(params.get("id")).catch(() => null);
        if (!asset) {
...
//# sourceMappingURL=asset-manager.routes-BXLTLQXC.js.map
