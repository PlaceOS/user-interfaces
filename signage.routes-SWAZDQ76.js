...
var ZONE_LIST = {};
var EMPTY_ZONE = new Xt();
var ZonePipe = class _ZonePipe {
  async transform(id) {
    if (ZONE_LIST[id])
      return ZONE_LIST[id];
    const zone = await ah(id).catch((_) => null);
    if (!zone)
      return EMPTY_ZONE;
    ZONE_LIST[id] = zone;
    return zone;
...
var SignageApprovePlaylistModalComponent = class _SignageApprovePlaylistModalComponent {
  ...
  async undoChanges() {
    ...
    await el(this._playlist?.id, previous.items);
    this.loading.set("");
    this._dialog_ref.disableClose = false;
...
var SignageDisplayModalComponent = class _SignageDisplayModalComponent {
  ...
  async save() {
    ...
    const new_display = new Rs(__spreadProps(__spreadValues({}, form_value), {
    ...
    const method = this.display.id ? $a(this.display.id, new_display) : va(new_display);
...
var SignageMediaModalComponent = class _SignageMediaModalComponent {
  ...
  async saveMedia() {
    ...
    if (this.item.id) {
      await Hh(this.item.id, new_media).catch(onError);
    } else {
...
var SignagePlaylistModalComponent = class _SignagePlaylistModalComponent {
  constructor() {
    this._data = inject(MAT_DIALOG_DATA) ?? new _r({});
...
var SignageStateService = class _SignageStateService extends AsyncHandler {
  constructor() {
    ...
    this._media = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_media" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const list = await queryAllPages(Mh({ limit: 200 }));
    ...
    this._playlists = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_playlists" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const list = await queryAllPages(Gh({ limit: 200 }));
    ...
    this._displays = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_displays" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const list = await queryAllPages(ga({
          zone_id: (this._settings.get("app.use_region") ? params.region : "") || params.building,
          limit: 200,
          signage: true
        }));
        return list.sort((a, b) => (a.display_name || a.name).localeCompare(b.display_name || b.name)).filter((_) => _.signage);
      }
    }));
    ...
    this._zones = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_zones" } : (
      ...
      defaultValue: [],
      loader: async () => {
        const list = await queryAllPages(uh({
          limit: 200,
          tags: "signage"
        })).catch(() => []);
  ...
  async editPlaylist(playlist = new _r({})) {
  ...
  async editMedia(media = new ws({}), file, playlist_id = "") {
  ...
  async editDisplay(display = new Rs({})) {
  ...
  async savePlaylist(playlist) {
    ...
    const call = clean_playlist.id ? Wh(clean_playlist.id, clean_playlist, "put") : Qh(clean_playlist);
  ...
  async getPlaylistMedia(playlist_id) {
    const { items } = await Zh(playlist_id);
    return items;
  }
  previewMedia(item) {
  ...
  previewFileMedia(media, playlist_id = "") {
    ...
    ref.componentInstance.save.subscribe(async () => {
      ...
      if (playlist_id && new_media.id) {
        ...
        await el(playlist_id, new_media_list);
      }
      ref.close();
    });
  }
  async addMediaFromLink(url, media_item = new ws({})) {
  ...
  constructor() {
    ...
    this._zones = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_zones" } : (
      ...
      loader: async ({ params: term }) => {
        const resp = await uh(__spreadProps(__spreadValues({}, this.query), {
          q: term,
          limit: 100
        })).catch(() => ({ data: [] }));
        return resp.data;
...
var SignageDisplaysComponent = class _SignageDisplaysComponent extends AsyncHandler {
  ...
  async setOrientation(orientation) {
    ...
    if (!display)
      return;
    await $a(display.id, { orientation, version: display.version }, "patch").catch((e) => {
...
var SignageMediaListComponent = class _SignageMediaListComponent {
  ...
  async addToPlaylist(media_id, playlist) {
    const media_list = await Zh(playlist.id);
  ...
  constructor() {
    ...
    this._playlist_media_resource = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_playlist_media_resource" } : (
      ...
      loader: async ({ params }) => {
        ...
        this.loading.set(true);
        try {
          const result = await Zh(params.id).catch(() => ({ id: "", items: [], approved: 0 }));
...
//# debugId=791f3628-f9d2-53a7-af4d-46549b96104b
//# sourceMappingURL=signage.routes-SWAZDQ76.js.map
