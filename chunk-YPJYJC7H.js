...
var BookingRulesFormComponent = class _BookingRulesFormComponent {
  constructor() {
    ...
    this.building_zones = toObservable(this._org.active_building).pipe(filter((_) => !!_), switchMap((bld) => Ba({ parent_id: bld.id }).catch(() => ({ data: [] }))), map((res) => res.data), shareReplay(1));
...
function BookingRulesModalComponent_Case_8_For_21_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance(5);
    \u0275\u0275conditional(\u0275\u0275pipeBind1(10, 22, row_r5.zone)?.id ? 9 : 11);
    \u0275\u0275advance(4);
...
var BookingRulesModalComponent = class _BookingRulesModalComponent {
  constructor() {
    ...
    this._booking_rules = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_booking_rules" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const { details } = await ju(params.building, `${this._data.type}_booking_rules`).catch(() => ({ details: [] }));
  ...
  async removeRuleset(ruleset) {
    ...
    if (index >= 0) {
      ...
      await Gu(this._org.building.id, {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(BookingRulesModalComponent, { className: "BookingRulesModalComponent", filePath: "apps/concierge/src/app/ui/booking-rules-modal.component.ts", lineNumber: 409 });
})();

export {
  BookingRulesModalComponent
};
//# sourceMappingURL=chunk-YPJYJC7H.js.map
