import {
  FAV_LOCKER_KEY
} from "./chunk-ZB45QX5E.js";
import "./chunk-AS2UUM4V.js";
import {
  BookingCardComponent,
  EventCardComponent
} from "./chunk-U7M6NTZ5.js";
import "./chunk-QEWFNKBW.js";
import {
  FAV_DESK_KEY
...
import {
  FAV_PARKING_KEY
} from "./chunk-5RCI73IE.js";
import "./chunk-BNMZ4E2X.js";
import "./chunk-ZL4MQP34.js";
import {
  BookingFormService
...
import "./chunk-6VWXOFF3.js";
import "./chunk-OFV5AHHV.js";
...
var LandingStateService = class _LandingStateService extends AsyncHandler {
  constructor() {
    ...
    this.search_fn = async (q) => this._settings.get("app.basic_user_search") || this._settings.get("app.colleagues_require_auth") !== false ? Fa({ q, authority_id: It()?.id }).then(({ data }) => data.map((_) => new StaffUser(_))) : searchStaff(q);
    ...
    effect((onCleanup) => {
      ...
      for (const [idx, space] of list.entries()) {
        const binding = fd(space.id, "Bookings").variable("status");
  ...
  async updateContacts() {
    const metadata = await Vu(currentUser().id, "contacts");
    const list = metadata.details instanceof Array ? metadata.details : [];
    const users = await Promise.all(list.map((_) => La(_.email).catch(() => _)));
...
var LandingFavouritesComponent = class _LandingFavouritesComponent extends AsyncHandler {
  ...
  async ngOnInit() {
    const metadata = await Vu(this._org.organisation.id, "room_alerts");
...
//# sourceMappingURL=landing.routes-GFOITVGX.js.map
