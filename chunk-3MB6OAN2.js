...
var OrganisationService = class _OrganisationService {
  ...
  async _bulkMetadataDetails(name, ids) {
    ...
    const metadata = await hc(name, { parent_ids }).catch((err) => err?.status === 404 ? this._individualMetadata(name, ids) : {});
...
var RemoteLoggingService = class _RemoteLoggingService extends AsyncHandler {
  ...
  _bindTo(id, name, mod = "Logger") {
    const module2 = Dd(id, mod).variable(name);
...
//# debugId=22e180a9-a880-50cd-9a48-2529a0003cb7
//# sourceMappingURL=chunk-3MB6OAN2.js.map
