...
var SelectUserModalComponent = class _SelectUserModalComponent {
  constructor() {
    ...
    this._users = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_users" } : (
      ...
      loader: ({ params: { q } }) => Ma({ q }).then((o) => o.data.map((u) => new StaffUser(u))).catch(() => [])
    }));
    this.users = computed(
      () => this._users.value() ?? [],
...
var LockerStateService = class _LockerStateService extends AsyncHandler {
  ...
  async checkinLocker(locker, state = true) {
    const status = await checkinBooking(locker.id, state ?? true).catch((_) => ({ failed: true, error: _ }));
    if (status.failed) {
...
//# sourceMappingURL=lockers.routes-MAPRROJY.js.map
