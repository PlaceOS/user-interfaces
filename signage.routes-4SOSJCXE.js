...
var ZonePipe = class _ZonePipe {
  async transform(id) {
    if (ZONE_LIST[id])
      return ZONE_LIST[id];
    const zone = await lh(id).catch((_) => null);
    if (!zone)
      return EMPTY_ZONE;
    ZONE_LIST[id] = zone;
    return zone;
...
var SignageApprovePlaylistModalComponent = class _SignageApprovePlaylistModalComponent {
  ...
  async undoChanges() {
    ...
    await nl(this._playlist?.id, previous.items);
    this.loading.set("");
    this._dialog_ref.disableClose = false;
...
var SignageDisplayModalComponent = class _SignageDisplayModalComponent {
  ...
  async save() {
    ...
    const method = this.display.id ? ka(this.display.id, new_display) : Sa(new_display);
...
var SignageStateService = class _SignageStateService extends AsyncHandler {
  constructor() {
    ...
    this._media = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_media" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const list = await queryAllPages(Oh({ limit: 200 }));
    ...
    this._playlists = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_playlists" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const list = await queryAllPages(Wh({ limit: 200 }));
    ...
    this._displays = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_displays" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const list = await queryAllPages($a({
    ...
    this._zones = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_zones" } : (
      ...
      defaultValue: [],
      loader: async () => {
        const list = await queryAllPages(ah({
          limit: 200,
          tags: "signage"
        })).catch(() => []);
  ...
  async savePlaylist(playlist) {
    ...
    const call = clean_playlist.id ? Kh(clean_playlist.id, clean_playlist, "put") : Zh(clean_playlist);
  ...
  async getPlaylistMedia(playlist_id) {
    const { items } = await Vh(playlist_id);
    return items;
  }
  previewMedia(item) {
  ...
  previewFileMedia(media, playlist_id = "") {
    ...
    ref.componentInstance.save.subscribe(async () => {
      ...
      if (playlist_id && new_media.id) {
        ...
        await nl(playlist_id, new_media_list);
      }
      ref.close();
    });
  }
  ...
  constructor() {
    ...
    this._zones = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_zones" } : (
      ...
      loader: async ({ params: term }) => {
        const resp = await ah(__spreadProps(__spreadValues({}, this.query), {
          q: term,
          limit: 100
        })).catch(() => ({ data: [] }));
        return resp.data;
...
var SignageDisplaysComponent = class _SignageDisplaysComponent extends AsyncHandler {
  ...
  async setOrientation(orientation) {
    ...
    if (!display)
      return;
    await ka(display.id, { orientation, version: display.version }, "patch").catch((e) => {
...
var SignageMediaListComponent = class _SignageMediaListComponent {
  ...
  async addToPlaylist(media_id, playlist) {
    const media_list = await Vh(playlist.id);
  ...
  constructor() {
    ...
    this._playlist_media_resource = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_playlist_media_resource" } : (
      ...
      loader: async ({ params }) => {
        ...
        this.loading.set(true);
        try {
          const result = await Vh(params.id).catch(() => ({ id: "", items: [], approved: 0 }));
...
//# sourceMappingURL=signage.routes-4SOSJCXE.js.map
