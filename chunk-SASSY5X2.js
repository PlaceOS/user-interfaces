...
function getCateringRulesForZone(zone_id, fresh = false) {
  ...
  if (!RULE_REQUESTS[zone_id] || fresh)
    RULE_REQUESTS[zone_id] = Pu(zone_id, "catering_config").pipe(map((_) => _.details instanceof Array ? _.details : []), catchError((e) => of([])));
...
var _CateringStateService = class _CateringStateService extends AsyncHandler {
  ...
  constructor() {
    ...
    this.settings = combineLatest([
      this._org.active_building,
      this._change
    ]).pipe(filter(([_]) => !!_), switchMap(([_]) => Pu(_.id, "catering-settings").pipe(catchError((_2) => of({})))), map((_) => _.details || {}), tap((_) => this._settings.post("require_catering_notes", !!_?.require_notes)), shareReplay(1));
  ...
  updateMenu(zone_id, menu) {
    return lastValueFrom(Tu(zone_id, {
      id: zone_id,
      name: "catering",
      details: menu,
  ...
  async saveSettings(settings) {
    ...
    const result = await lastValueFrom(Tu(this._org.building.id, {
  ...
  async getCateringForZone(zone_id) {
    const menu = (await Pu(zone_id, "catering").toPromise()).details;
    return menu instanceof Array ? menu : [];
  }
  async getCateringConfig(zone_id = this._org.building.id) {
    const rules = (await Pu(zone_id, "catering_config").toPromise()).details;
...
var _CateringOrderStateService = class _CateringOrderStateService {
  constructor() {
    ...
    this.settings = this._org.active_building.pipe(filter((_) => !!_), switchMap((_) => Pu(_.id, "catering-settings").pipe(catchError((_2) => of({})))), map((_) => _.details), tap((_) => this._settings.post("require_catering_notes", !!_?.require_notes)), shareReplay(1));
    this.charge_codes = this.settings.pipe(map((_) => _.charge_codes || []));
    this.availability = this.settings.pipe(map((_) => _.disabled_rooms || []));
    this.available_menu = combineLatest([
      ...
      return Pu(zone || bld.id, "catering").pipe(map((d) => (d.details instanceof Array ? d.details : []).map((_) => new CateringItem(_))), catchError((_) => []));
    }), tap((items) => {
...
//# sourceMappingURL=chunk-SASSY5X2.js.map
