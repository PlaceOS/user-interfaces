...
} from "./chunk-O4MJWGUI.js";
import {
  BookingHistoryModalComponent
} from "./chunk-AVFZMPPT.js";
import {
...
// apps/concierge/src/app/desks/desk-booking-actions.ts
function canChangeDeskBooking(booking) {
  return !(booking.deleted || isDeskBookingRejected(booking) || booking.has_ended || booking.status === "cancelled" || booking.status === "ended");
}
function isDeskBookingRejected(booking) {
...
var DesksStateService = class _DesksStateService extends AsyncHandler {
  ...
  async checkinDesk(desk, state = true) {
    if (!canChangeDeskBooking(this._normaliseBooking(desk)))
      return;
  ...
  async cancelBooking(booking, series = false) {
    if (isDeskBookingRejected(this._normaliseBooking(booking)))
      return;
  ...
  async rejectAllDesks() {
    const list = this.bookings().filter((desk) => canChangeDeskBooking(this._normaliseBooking(desk)));
    if (list.length <= 0)
...
//# debugId=9bdbe2e2-02fa-5b7e-ba12-00d7f9d929d5
//# sourceMappingURL=chunk-IOCXVZTD.js.map
