...
} from "./chunk-2BBWVXRL.js";
import {
  DateFieldComponent
} from "./chunk-XCO2V47B.js";
import "./chunk-PD5M6XCZ.js";
import {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AssetCategoryFormComponent, { className: "AssetCategoryFormComponent", filePath: "apps/concierge/src/app/asset-manager/asset-category-form.component.ts", lineNumber: 111 });
})();
...
var AssetManagerStateService = class _AssetManagerStateService extends AsyncHandler {
  constructor() {
    ...
    this._settings_data = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_settings_data" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return {};
        const metadata = await ic(params.building, "assets-settings").catch(() => ({}));
  ...
  async postForm() {
    ...
    data.other_data = oi(other_data, [void 0, null, ""]);
  ...
  async getConfig(zone_id = this._org.building.id) {
    const rules = (await ic(zone_id, "assets_config")).details;
...
var AssetBulkFormComponent = class _AssetBulkFormComponent extends AsyncHandler {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        ...
        const asset = await Ll(params.get("id")).catch(() => null);
        if (!asset) {
      ...
      if (params.get("group_id")) {
        ...
        const product = await Jl(params.get("group_id")).catch(() => null);
        if (!product) {
  ...
  async save() {
    ...
    const data = this.model();
    const list = await saveAssetsInBulk(new Array(this.count()).fill(__spreadProps(__spreadValues({}, data), {
      zone_id: this._org.building.id
    }))).catch((e) => {
      this.loading.set("");
...
var AssetGroupFormComponent = class _AssetGroupFormComponent extends AsyncHandler {
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        this.loading.set("Loading Product Details...");
        const product = await Jl(params.get("id")).catch(() => null);
        if (!product) {
...
var AssetManagerTopbarComponent = class _AssetManagerTopbarComponent extends AsyncHandler {
  ...
  setBuilding(id2) {
    const bld = this._org.buildings.find((_) => _.id === id2);
    if (!bld)
      return;
  ...
  ngOnInit() {
    this.subscription("route.query", this._route.queryParamMap.subscribe(async (params) => {
      if (params.get("id")) {
        ...
        const asset = await od(params.get("id")).catch(() => null);
        if (!asset) {
...
var AssetViewComponent = class _AssetViewComponent extends AsyncHandler {
  constructor() {
    ...
    this._removed_asset_ids = signal(
      /* @__PURE__ */ new Set(),
      ...ngDevMode ? [{ debugName: "_removed_asset_ids" }] : (
        /* istanbul ignore next */
        []
      )
    );
    this.asset_list = computed(
      () => {
        ...
        ], "id").filter((asset) => !removed_asset_ids.has(asset.id));
      },
  ...
  async removeAsset(asset) {
    ...
    this._removed_asset_ids.update((removed_asset_ids) => /* @__PURE__ */ new Set([...removed_asset_ids, asset.id]));
    this._state.setExtraAssets(this.extra_assets().filter((item) => item.id !== asset.id));
    this._state.postChange();
    resp.close();
  }
...
//# debugId=d90e5351-1b6d-5959-abdb-3e1e260dcc1b
//# sourceMappingURL=asset-manager.routes-JMW7UG6W.js.map
