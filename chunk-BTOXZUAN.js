...
var SpacePipe = class _SpacePipe {
  ...
  async transform(space_id) {
    ...
    if (ATTEMPT_COUNT[space_id])
      return EMPTY_SPACE;
    if (!is_email) {
      const system = await ba(space_id).catch((_) => null);
      if (system) {
...
function generateEventForm(event = new CalendarEvent(), settings, injector) {
  ...
  onFieldChange(model, (v2) => v2.organiser, (organiser) => (
  ...
  onFieldChange(model, (v2) => v2.resources, (resources) => model.update((m) => __spreadProps(__spreadValues({}, m), {
    system: resources?.length ? resources[0] : null
  })), injector);
  onFieldChange(model, (v2) => v2.date, (date) => {
  ...
  onFieldChange(model, (v2) => v2.catering, setCateringTime, injector);
...
//# sourceMappingURL=chunk-BTOXZUAN.js.map
