...
var PlaylistApproveModalComponent = class _PlaylistApproveModalComponent {
  constructor() {
    ...
    this._playlist_versions = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_playlist_versions" } : (
      ...
      loader: async ({ params }) => {
        ...
        try {
          const versions = await Yh(params, { limit: 2 });
  ...
  async undoChanges() {
    ...
    try {
      await nl(this._data.playlist.id, previous_version.items);
  ...
  async approve() {
    ...
    this._dialog_ref.disableClose = true;
    try {
      await Xh(this._data.playlist.id);
...
//# sourceMappingURL=playlist-approve-modal.component-5BZBCXRQ.js.map
