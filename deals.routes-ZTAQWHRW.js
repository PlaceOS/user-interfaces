...
var DealsService = class _DealsService extends AsyncHandler {
  constructor() {
    ...
    this._deals = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_deals" } : (
      ...
      loader: async ({ params }) => {
        ...
        const metadata = await rc(params.building, "deals-n-offers").catch(() => ({ details: [] }));
  ...
  async getDeals() {
    const metadata = await rc(this._org.building.id, "deals-n-offers").catch(() => null);
    return metadata?.details instanceof Array ? metadata.details : [];
  }
  async saveDeal(deal) {
    const metadata = await rc(this._org.building.id, "deals-n-offers").catch(() => null);
...
//# sourceMappingURL=deals.routes-ZTAQWHRW.js.map
