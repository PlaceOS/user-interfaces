...
var _GuestListingComponent = class _GuestListingComponent extends AsyncHandler {
  ...
  ngOnInit() {
    this.subscription("building", this._org.active_building.subscribe(async (bld) => {
      ...
      const metadata = await lastValueFrom(Xu(bld.id, visitor_kiosk_app));
      const org_metadata = await lastValueFrom(Xu(this._org.organisation.id, visitor_kiosk_app));
...
//# sourceMappingURL=visitors.module-AJZD2VXE.js.map
