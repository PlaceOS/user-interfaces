...
var BookingFormService = class _BookingFormService extends AsyncHandler {
  ...
  newForm(type, booking = new Booking({})) {
    ...
    this.form().reset();
    this._patch(Ys(__spreadValues(__spreadValues({}, booking.extension_data), booking), [null, void 0, ""]), { emitEvent: false });
  ...
  resetForm() {
    ...
    this.form().reset();
    this._patch(Ys(__spreadValues(__spreadValues({}, booking || {}), booking?.extension_data || {}), [null, void 0, ""]));
    this._options.set({ type: this._options().type });
  }
  clearForm() {
    this._clearStoredForm();
  ...
  loadForm() {
    ...
    this.form().reset();
    const booking_data = Ys(__spreadValues(__spreadValues(__spreadValues({}, data), booking || {}), booking?.extension_data || {}), [null, void 0, ""]);
  ...
  async postForm(ignore_check = false, reset_form = true) {
    ...
    if (ignore_check) {
      await this._checkAssignedDeskRestriction(host, this._options().type);
    } else {
...
//# sourceMappingURL=chunk-WWS5253B.js.map
