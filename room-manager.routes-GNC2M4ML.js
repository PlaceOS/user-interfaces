...
var RoomAlertModalComponent = class _RoomAlertModalComponent {
  ...
  async save() {
    ...
    const metadata = await Vu(this._org.organisation.id, "room_alerts").catch((e) => {
...
var RoomModalComponent = class _RoomModalComponent extends AsyncHandler {
  ...
  async ngOnInit() {
    const { details } = await Vu(this._org.organisation.id, "settings");
  ...
  async save() {
    ...
    await (data.id ? fa(data.id, data) : _a(data));
...
var RoomManagementService = class _RoomManagementService {
  constructor() {
    ...
    this._room_alerts = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_room_alerts" } : (
      ...
      loader: async () => {
        const metadata = await Vu(this._org.organisation.id, "room_alerts").catch(() => ({ details: {} }));
    ...
    this._room_list = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_room_list" } : (
      ...
      loader: async ({ params }) => {
        ...
        const list = await queryAllPages(la({ zone_id, limit: 200 })).catch(() => []);
...
//# debugId=d9db821f-e706-5dba-ac8f-a78aa0720df7
//# sourceMappingURL=room-manager.routes-GNC2M4ML.js.map
