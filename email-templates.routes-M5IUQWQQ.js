...
var EmailTemplatesStateService = class _EmailTemplatesStateService extends AsyncHandler {
  constructor() {
    ...
    this._template_definitions = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_template_definitions" } : (
      ...
      loader: async ({ params }) => {
        if (!params.building)
          return [];
        const metadata = await ic(this._org.organisation.id, "email_template_fields").catch(() => null);
  ...
  async _queryTemplates(building, region) {
    ...
    const [org_templates, bld_templates, region_templates] = await Promise.all([
      ic(this._org.organisation.id, "email_templates").then((_) => this._processTemplates(_, this._org.organisation.id)).catch(() => []),
      ic(building, "email_templates").then((_) => this._processTemplates(_, building)).catch(() => []),
      region ? ic(region, "email_templates").then((_) => this._processTemplates(_, region)).catch(() => []) : Promise.resolve([])
    ]);
  ...
  async saveTemplate(template, old_zone = "") {
    ...
    if (template.id && old_zone) {
      const old_metadata = await ic(old_zone, "email_templates");
    ...
    const metadata = await ic(template.zone_id, "email_templates");
...
//# debugId=8efafb8a-0074-5dd4-aa65-6db1bddda411
//# sourceMappingURL=email-templates.routes-M5IUQWQQ.js.map
