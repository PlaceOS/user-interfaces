...
var _SurveyService = class _SurveyService {
  ...
  async setSurvey(id) {
    ...
    const survey = await lastValueFrom(Ca(id));
  ...
  async removeSurvey(survey, confirm = true) {
    if (confirm) {
      ...
      await lastValueFrom(za(`${survey.id}`));
      result.close();
    } else {
      await lastValueFrom(za(`${survey.id}`));
    }
  ...
  async removeQuestion(question, confirm = true) {
    if (confirm) {
      ...
      await lastValueFrom(qa(`${question.id}`));
      result.close();
    } else {
      await lastValueFrom(qa(`${question.id}`));
    }
  ...
  async _load(type = LoadType.ALL) {
    ...
    if (type === LoadType.ALL || type === LoadType.SURVEYS) {
      const surveys = await lastValueFrom(forkJoin(buildings.map((bld) => Na({ building_id: bld.id }))));
      const survey_list = surveys.flat();
      this.survey_list.set(survey_list);
    }
    if (type === LoadType.ALL || type === LoadType.ANSWERS) {
      const answers = await lastValueFrom(forkJoin(this.survey_list().map((survey) => Aa({ survey_id: survey.id }))));
      this.answer_list.set(answers.flat());
    }
    if (type === LoadType.ALL || type === LoadType.QUESTIONS) {
      const questions = await lastValueFrom(Ma({ limit: 1e3 }));
...
var _QuestionComponent = class _QuestionComponent {
  constructor() {
    ...
    this.question = model(new Zt({ title: "", type: QuestionType.Single_Line_Text }));
...
var _QuestionModalComponent = class _QuestionModalComponent {
  ...
  ngOnInit() {
    ...
    this.question = this._data || new Zt({ type: "text" });
  ...
  async save() {
    ...
    if (!question_el.valid)
      return;
    this.loading = true;
    const call = this.is_edit ? Ua(`${this.question.id}`, question_el.question()) : Da(question_el.question());
...
var _NewSurveyService = class _NewSurveyService {
  constructor() {
    ...
    this.survey_list$ = combineLatest([
      ...
      this._change
    ]).pipe(tap(() => this._loading.next(true)), switchMap(([list]) => forkJoin(list.map((bld) => Na({ building_id: bld.id })))), map((data) => flatten(data)), tap((list) => this.survey_list.set(list)), shareReplay(1));
    this.answer_list$ = this.survey_list$.pipe(switchMap((surveys) => forkJoin(surveys.map((survey) => Aa({ survey_id: survey.id })))), map((answers) => flatten(answers)), tap(() => this._loading.next(false)), tap((list) => this.answer_list.set(list)), shareReplay(1));
    ...
    this.survey$ = this._survey_id.pipe(switchMap((id) => !id ? of(null) : Ca(id)), shareReplay(1));
    this.questions$ = Ma({
      limit: 1e3
    }).pipe(
  ...
  editQuestion(question = new Zt({ type: "text" })) {
...
var _QuestionPipe = class _QuestionPipe extends AsyncHandler {
  transform(id) {
    if (!QUESTION_MAP[id]) {
      lastValueFrom(Ra(id)).then((question) => QUESTION_MAP[id] = question);
    }
...
var _SurveyBuilderComponent = class _SurveyBuilderComponent extends AsyncHandler {
  ...
  async saveSurvey() {
    ...
    const call = this.form.value.id ? Fa(survey) : ja(`${survey.id}`, survey);
...
var _SurveyResponsesComponent = class _SurveyResponsesComponent extends AsyncHandler {
  constructor() {
    ...
    this.answers$ = combineLatest([
      ...
      return Aa(q).pipe(catchError(() => of([])));
...
//# sourceMappingURL=surveys.module-CYQO66PQ.js.map
