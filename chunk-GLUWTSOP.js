...
} from "./chunk-NCJVEKWN.js";
import {
  BookingHistoryModalComponent
} from "./chunk-MWU7FM7W.js";
import {
...
var DesksStateService = class _DesksStateService extends AsyncHandler {
      ...
      loader: async ({ params }) => {
        ...
        try {
          ...
          if (zones.length && !all_zones) {
            const metadata = await Wu(zones[0], "desks").catch(() => ({ details: [] }));
            list = (metadata.details instanceof Array ? metadata.details : []).map((i) => __spreadProps(__spreadValues({}, i), { zone: { id: zones[0] } }));
          } else {
            const metadata = await Ju(this._org.building?.id, { name: "desks" }).catch(() => []);
  ...
  async _checkAssignedDeskLimit(user_email, current_desk_id) {
    ...
    const assigned_count = (await Promise.all(this._currentLevelList().map((level) => Wu(level.id, "desks").then((metadata) => metadata.details instanceof Array ? metadata.details : []).catch(() => [])))).flat().filter((item) => item.id !== current_desk_id && item.assigned_to?.toLowerCase() === email).length;
...
//# sourceMappingURL=chunk-GLUWTSOP.js.map
