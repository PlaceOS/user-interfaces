...
import {
  LevelPipe
} from "./chunk-URHUNZ6M.js";
import {
  getMapDetails
} from "./chunk-DMUJUXBA.js";
import {
  openConfirmModal
...
var AutoReleaseSettingsModalComponent = class _AutoReleaseSettingsModalComponent {
  ...
  async loadSettings(id) {
    ...
    const settings = (await ra({ parent_id: id })).data;
  ...
  async save() {
    ...
    const settings = (await ra({ parent_id: this.id })).data;
    ...
    unencrypted.id ? await ua(unencrypted.id, unencrypted).catch(on_error) : await ca(unencrypted).catch(on_error);
...
var BuildingFormComponent = class _BuildingFormComponent extends AsyncHandler {
  ...
  async saveChanges() {
    ...
    const building = await (data.id ? ih(data.id, body) : rh(body)).catch((e) => {
...
var BookingPanelSettingsModalComponent = class _BookingPanelSettingsModalComponent extends AsyncHandler {
  ...
  async ngOnInit() {
    ...
    let settings = [];
    try {
      const response = await ra({ parent_id: this.zone.id });
  ...
  async save() {
    ...
    const update = unencrypted_settings.id ? ua(unencrypted_settings.id, new_setting) : ca(new_setting);
    try {
      await update;
    } catch (e) {
      this._dialog_ref.disableClose = false;
...
var LevelMapEditorModalComponent = class _LevelMapEditorModalComponent {
  ...
  async save() {
    ...
    try {
      ...
      const response = await ih(this._level.id, {
        id: this._level.id,
...
var LevelModalComponent = class _LevelModalComponent {
  ...
  async save() {
    ...
    const resp = await (data.id ? ih(data.id, __spreadProps(__spreadValues({}, data), {
      name: `LEVEL ${It().description} ${data.display_name}`
    })) : rh(__spreadProps(__spreadValues({}, data), {
      name: `LEVEL ${It().description} ${data.display_name}`
    }))).catch();
    if (resp.id)
...
//# debugId=b8e35714-6a07-54e7-b2d4-e4a49ae10c3a
//# sourceMappingURL=zone-manager.routes-EIS6SVPG.js.map
